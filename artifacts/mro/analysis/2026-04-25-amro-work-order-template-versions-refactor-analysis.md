# AMRO Schema Refactor Analysis: `amro_work_order_template_versions` -> `amro_work_order_template_versions`

## Scope
- Date: `2026-04-25`
- Requested refactor:
  - Table rename: `public.amro_work_order_template_versions` -> `public.amro_work_order_template_versions`
  - Rename associated constraints/indexes/policies to work-order naming conventions
  - Repoint inbound FKs from `amro_work_order_template_versions(id)` to `amro_work_order_template_versions(id)`
...
- Repository-wide scan found no static FK DDL targeting:
  - `public.amro_work_order_template_versions(id)`
  - `amro_work_order_template_versions(id)`
...
- Updated API route handlers to use canonical table name:
  - `src/pages/api/v2/amro/work-order-template-versions/index.ts`
  - `src/pages/api/v2/amro/work-order-template-versions/[id].ts`
  - `src/pages/api/v2/amro/work-order-template-versions/[id]/submit.ts`
  - `src/pages/api/v2/amro/work-order-template-versions/[id]/approve.ts`
...
1. Existence checks:
   ...
   - `select to_regclass('public.amro_work_order_template_versions');` (expected null after migration)
...
3. FK target check:
   - `select conrelid::regclass as table_name, conname, pg_get_constraintdef(oid) as definition from pg_constraint where contype='f' and (pg_get_constraintdef(oid) ilike '%amro_work_order_template_versions%' or pg_get_constraintdef(oid) ilike '%amro_work_order_template_versions%');`
