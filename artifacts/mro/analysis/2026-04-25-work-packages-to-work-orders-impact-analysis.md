...
- Table rename: `public.work_packages` -> `public.work_orders`
- Supporting table rename: `public.work_packages_title` -> `public.work_order_titles`
- Column rename (schema-wide in `public`):
  - `work_package_template_id` -> `work_order_template_id`
  - `work_package_title_id` -> `work_order_title_id`
- Domain rename:
  - `public.work_package_status` -> `public.work_order_status`
- Backward compatibility:
  - Compatibility domain alias recreated: `public.work_package_status`
  - Compatibility views created:
    - `public.work_packages` (maps old column aliases to `work_orders`)
    - `public.work_packages_title` (maps to `work_order_titles`)

## Impact Inventory Summary

- DB schema migrations:
  - New forward migration: `supabase/migrations/20260425160000_amro_rename_work_packages_to_work_orders.sql`
  - New rollback script: `scripts/sql/rollback_20260425160000_work_orders_to_work_packages.sql`
- API layer updates:
...
1. Run `scripts/sql/rollback_20260425160000_work_orders_to_work_packages.sql`.
...
2. Confirm compatibility objects:
   - `select to_regclass('public.work_packages');`
   - `select to_regclass('public.work_packages_title');`
