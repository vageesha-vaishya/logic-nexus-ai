...
- `src/features/module-amro/settings/pages/amro-settings-master-data/components/AircraftUnifiedLayout.test.tsx`
  ...
  - Verifies hidden/empty configuration does not render phantom layout containers
...
- `tests/e2e/amro-overview-dashboard.spec.ts`
  - Adds unified aircraft module rail traversal test for:
    ...
    - ad/sb
    - maintenance planning
  - Adds whitespace regression validation:
    - Ensures no empty `aircraft-unified-layout` wrapper appears under page title
    - Confirms "Aircraft Operations Snapshot" remains first visible content panel
    - Captures responsive evidence for desktop/tablet/mobile viewports
    - Captures browser-specific screenshots per module

## 9) Whitespace Fix Record

### 9.1 Root Cause

- A placeholder `AircraftUnifiedLayout` instance was rendered for aircraft sub-modules with:
  - all visual sections disabled (`showHeaderSummary`, `showNavRail`, `showSearchControls`, `showResultSummary`, `showLocaleSelector`, `showDynamicFilters`, `showActions`, `showClearFilters` all false)
  - empty children (`<></>`)
- The wrapper still produced a panel container below the page title, creating the persistent blank strip/whitespace artifact.

### 9.2 Unified Module-Wide Fix

- `src/features/module-amro/settings/pages/amro-settings-master-data/components/AircraftUnifiedLayout.tsx`
  - Added render guard: returns `null` when both header and body are effectively empty.
  - Added conditional `CardHeader` and `CardContent` rendering to avoid empty panel chrome.
  - Added robust child-visibility evaluation, including empty React fragments.
- The fix applies across all aircraft forms using the shared layout wrapper, preserving spacing for non-empty panels.

### 9.3 Affected Aircraft Form Templates
...
- Documents
- AD/SB
- Maintenance Planning (Work Packages/Operations)

### 9.4 Visual Evidence Paths

- Before fix baseline path:
  - `artifacts/mro/screenshots/whitespace-before/<browser>/<viewport>/<module>.png`
- After fix path (generated by e2e run output artifacts):
  - `artifacts/mro/screenshots/whitespace-after/<browser>/<viewport>/<module>.png`

### 9.5 Regression Protocol

1. Run whitespace e2e test across browser projects:
   - `npx playwright test tests/e2e/amro-overview-dashboard.spec.ts --grep "whitespace-regression evidence" --project=chromium --project=msedge-latest --project=firefox --project=webkit`
2. Validate each browser run at desktop/tablet/mobile viewport matrix.
3. For each module route, confirm:
   - no `aircraft-unified-layout` wrapper is present
   - no blank strip appears below page title/header section
   - `Aircraft Operations Snapshot` appears as first intended content panel
   - `Unified module search` remains visible and interactive
4. Archive screenshots under before/after paths for release QA sign-off.

## 10) User Training Material
