# AMRO Schema Refactor Analysis: `amro_work_order_audit_log` -> `amro_work_order_audit_log`

## Scope
- Date: `2026-04-25`
- Requested refactor:
  - Table rename: `public.amro_work_order_audit_log` -> `public.amro_work_order_audit_log`
  - Rename associated constraints/indexes/policies to work-order naming
  - Repoint inbound FKs from `amro_work_order_audit_log(id)` to `amro_work_order_audit_log(id)`
...
2. Renames constraints and indexes from work-order to work-order naming.
...
- Repository-wide static scan found no FK DDL definitions referencing:
  - `public.amro_work_order_audit_log(id)`
  - `amro_work_order_audit_log(id)`
...
1. Existence checks:
   ...
   - `select to_regclass('public.amro_work_order_audit_log');` (expected null after migration)
...
3. FK target check:
   - `select conrelid::regclass as table_name, conname, pg_get_constraintdef(oid) as definition from pg_constraint where contype='f' and (pg_get_constraintdef(oid) ilike '%amro_work_order_audit_log%' or pg_get_constraintdef(oid) ilike '%amro_work_order_audit_log%');`
