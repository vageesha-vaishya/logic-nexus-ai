...
/**
 * FlyPal Directives Migration Function - v3
 * Fixed: Use correct postgres@v0.17.0 API with object-based queries
 */
...
async function processRecord(args: {
  ...
  await args.client.queryObject({ text: "BEGIN", args: [] });
  ...
  try {
    // Attempt to insert directive record
    ...
    // SUCCESS PATH: Update source record with success state
    await args.client.queryObject({
      text: `
        ...
        WHERE ctid = $1::tid
      `,
      args: [args.record.row_locator, directiveId],
    });
    ...
    await args.client.queryObject({ text: "COMMIT", args: [] });
    return { success: true, recordId };

  } catch (insertError: unknown) {
    // FAILURE PATH: Insert failed
    ...
    try {
      // Update source record with failure state (still in transaction)
      await args.client.queryObject({
        text: `
          UPDATE flypal.flypal_directives
          SET
          ...
          WHERE ctid = $1::tid
        `,
        args: [args.record.row_locator, reason],
      });
      ...
      // COMMIT failure transaction (with failure state recorded)
      await args.client.queryObject({ text: "COMMIT", args: [] });

    } catch (updateError: unknown) {
      // If even the failure update fails, still try to commit what we can
      try {
        await args.client.queryObject({ text: "COMMIT", args: [] });
      } catch {
        // If commit fails, at least try to rollback
        try {
          await args.client.queryObject({ text: "ROLLBACK", args: [] });
        } catch {
          /* no-op */
        }
      }
      ...
      const updateErrorMsg = updateError instanceof Error
        ? `${updateError.message}\n${updateError.stack || ""}`
        : String(updateError);
      ...
      return {
        ...
        recordId,
        reason: truncate(
          `Primary Error: ${reason}\n\nSecondary Error: ${updateErrorMsg}`,
          args.failureMaxLen
        ),
      };
    }
...
serveWithLogger(async (req, logger, supabaseAdmin) => {
  ...
  try {
    ...
    const tenantCheck = await rootClient.queryObject<{ exists: boolean }>({
      text: `SELECT EXISTS(SELECT 1 FROM public.tenants WHERE id = $1) AS exists`,
      args: [tenantId],
    });
    ...
    try {
      ...
      await logger.warn("Could not determine failure_reason column length", {
    ...
    await logger.error("Unhandled error in migrate-flypal-directives-v3", {
      error: errorMsg,
      tenant_id: tenantId,
      model_name: modelName,
    });
