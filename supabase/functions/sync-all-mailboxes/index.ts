...
serveWithLogger(async (req, logger, supabase) => {
  ...
  try {
    ...
    // supabase client is injected by serveWithLogger (Service Role)
    ...
    for (const acc of accounts || []) {
      try {
        ...
        if (res.ok) {
          ...
          logger.error(`Sync-emails call failed for ${acc.id}: ${await res.text()}`);
        }
      } catch (e) {
        logger.error(`Sync call error for ${acc.id}`, { error: e });
      }
    }
    ...
    logger.error("Error syncing all mailboxes:", { error });
    return new Response(
