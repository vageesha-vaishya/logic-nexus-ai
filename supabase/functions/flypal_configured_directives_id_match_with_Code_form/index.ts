...
serveWithLogger(async (req, logger, supabase) => {
  ...
  try {
    ...
    for (let i = 0; i < maxBatches; i += 1) {
      const { data, error } = await supabase.rpc("flypal_configured_directives_id_match_batch_with_code_form", {
    ...
    await logger.error("flypal_configured_directives_id_match_with_Code_form execution failed", { error: message });
    return new Response(JSON.stringify({
      error: message,
