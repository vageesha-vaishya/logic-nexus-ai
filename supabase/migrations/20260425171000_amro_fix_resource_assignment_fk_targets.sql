...
--
-- Purpose:
-- - Repoint any residual foreign keys referencing public.amro_work_order_resource_assignments(id)
...
BEGIN
  ...
  IF v_new_table IS NULL AND v_old_table IS NOT NULL THEN
    ...
    v_old_table := to_regclass('public.amro_work_order_resource_assignments');
  END IF;
  ...
  LOOP
    v_new_name := replace(rec.conname, 'work_order_resource_assignments', 'work_order_resource_assignments');
    ...
    v_def := replace(
      replace(rec.condef, 'REFERENCES public.amro_work_order_resource_assignments', 'REFERENCES public.amro_work_order_resource_assignments'),
