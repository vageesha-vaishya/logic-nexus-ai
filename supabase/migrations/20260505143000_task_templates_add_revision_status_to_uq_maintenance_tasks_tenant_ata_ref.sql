-- DB-VERIFICATION: task-templates-uq-maintenance-tasks-tenant-ata-ref-add-revision-status-reviewed
...
BEGIN
  ...
  SELECT EXISTS (
    ...
    WHERE n.nspname = 'public'
      ...
      AND c.contype = 'u'
      AND pg_get_constraintdef(c.oid) = 'UNIQUE NULLS NOT DISTINCT (tenant_id, franchise_id, ata_code, assembly_models, reference_amp, code_form_no, category_code, description, revision_status)'
...
COMMENT ON CONSTRAINT uq_maintenance_tasks_tenant_ata_ref ON public.task_templates IS
  'Uniqueness scope set to tenant + franchise + ATA + assembly model + reference AMP + code_form_no + category_code + description + revision_status with NULLS NOT DISTINCT semantics.';
