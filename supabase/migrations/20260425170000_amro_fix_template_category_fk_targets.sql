...
--
-- Purpose:
-- - Repoint any residual foreign keys referencing public.amro_work_package_template_categories(id)
...
BEGIN
  ...
  IF v_new_table IS NULL AND v_old_table IS NOT NULL THEN
    ...
    v_old_table := to_regclass('public.amro_work_package_template_categories');
  END IF;
  ...
  LOOP
    v_new_name := replace(rec.conname, 'work_package_template_categories', 'work_order_template_categories');
    ...
    v_def := replace(
      replace(rec.condef, 'REFERENCES public.amro_work_package_template_categories', 'REFERENCES public.amro_work_order_template_categories'),
      'REFERENCES amro_work_package_template_categories',
