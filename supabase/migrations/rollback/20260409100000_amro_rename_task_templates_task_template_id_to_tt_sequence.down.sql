...
BEGIN
  ...
  IF has_new_column THEN
    ALTER TABLE public.task_templates
      RENAME COLUMN tt_sequence TO task_template_id;
  END IF;

  IF to_regclass('public.task_templates_tt_sequence_seq') IS NOT NULL
    AND to_regclass('public.task_templates_task_template_id_seq') IS NULL THEN
    ALTER SEQUENCE public.task_templates_tt_sequence_seq
  ...
  IF EXISTS (
    ...
    ALTER TABLE public.task_templates
      RENAME CONSTRAINT task_templates_tt_sequence_key TO task_templates_task_template_id_key;
  END IF;
  ...
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION 'Rollback verification failed: public.task_templates.task_template_id is not identity and NOT NULL';
  END IF;
END
$rollback$;
