-- Add enterprise data columns to work_order_templates
...
COMMENT ON COLUMN work_order_templates.materials_json IS 'Bill of Materials - Array of material line items with part numbers, quantities, costs, and supplier info';
COMMENT ON COLUMN work_order_templates.tooling_json IS 'Tooling & Equipment - Array of required tools with calibration and availability data';
COMMENT ON COLUMN work_order_templates.compliance_requirements_json IS 'Compliance Requirements - Array of AD/SB and regulatory requirements with sign-off tracking';
...
CREATE INDEX IF NOT EXISTS idx_work_order_templates_materials_count
ON work_order_templates USING btree (jsonb_array_length(materials_json));

CREATE INDEX IF NOT EXISTS idx_work_order_templates_tooling_count
ON work_order_templates USING btree (jsonb_array_length(tooling_json));

CREATE INDEX IF NOT EXISTS idx_work_order_templates_compliance_count
ON work_order_templates USING btree (jsonb_array_length(compliance_requirements_json));
