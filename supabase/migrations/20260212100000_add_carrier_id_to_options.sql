...
BEGIN
  ...
  -- 5. Determine Carrier from Selected Option (if any)
  ...
  -- If still null, try just the first option
  IF v_option IS NULL THEN
     SELECT qvo.* INTO v_option
     FROM public.quotation_versions qv
     JOIN public.quotation_version_options qvo ON qvo.quotation_version_id = qv.id
     WHERE qv.quote_id = p_quote_id 
     AND qv.is_active = true
     ORDER BY qvo.created_at DESC
     LIMIT 1;
  END IF;

  -- 6. Create Shipment Header
  INSERT INTO public.shipments(
    ...
    v_quote.notes,
    auth.uid(),
    v_option.carrier_id -- New value
  )
  RETURNING id INTO v_shipment_id;

  -- 7. Create Shipment Items (from Quote Items)
