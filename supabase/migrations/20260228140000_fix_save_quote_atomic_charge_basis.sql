...
BEGIN
  ...
  IF v_options IS NOT NULL AND jsonb_array_length(v_options) > 0 THEN
    ...
    LOOP
      ...
      -- NOTE: I noticed the original function didn't have combined charges loop in the snippet I read,
      -- but UnifiedQuoteComposer sends `combined_charges`. 
      -- If the previous function def didn't have it, I should add it if I want to persist them.
      -- The original snippet stopped at line 412, effectively ending the leg loop.
      -- If `combined_charges` persistence is missing, that's another bug!
      
      -- Let's check if I should add it.
      -- For now, I'll stick to fixing what was there, but if I want to be proactive...
      -- The `UnifiedQuoteComposer` sends `combinedPayload`.
      -- If `save_quote_atomic` ignores it, then global charges are lost.
      -- I should add support for `combined_charges` (charges with no leg_id).
      ...
      IF (v_opt -> 'combined_charges') IS NOT NULL AND jsonb_array_length(v_opt -> 'combined_charges') > 0 THEN
         ...
         LOOP
           ...
           -- Insert charge with NULL leg_id
           INSERT INTO quote_charges (
             leg_id, quote_option_id, tenant_id, category_id, note,
             basis_id, currency_id, quantity, rate, amount,
             charge_side_id, created_at, updated_at
           ) VALUES (
             NULL, v_option_id, v_tenant_id,
             (v_charge ->> 'charge_code')::uuid, 
             v_charge ->> 'note',
             v_basis_id,
             v_currency_id,
