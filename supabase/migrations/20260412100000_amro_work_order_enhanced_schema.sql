...
CREATE TABLE IF NOT EXISTS amro_work_order_template_versions (
...
CREATE INDEX idx_template_versions_tenant ON amro_work_order_template_versions(tenant_id);
CREATE INDEX idx_template_versions_template ON amro_work_order_template_versions(template_id);
CREATE INDEX idx_template_versions_status ON amro_work_order_template_versions(status);
CREATE INDEX idx_template_versions_effective ON amro_work_order_template_versions(effective_from, effective_until);

COMMENT ON TABLE amro_work_order_template_versions IS 'Template versioning with approval workflow for change control';

-- 1.2 Template categories and classification
CREATE TABLE IF NOT EXISTS amro_work_order_template_categories (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    tenant_id UUID NOT NULL,
...
CREATE INDEX idx_template_categories_tenant ON amro_work_order_template_categories(tenant_id);
CREATE INDEX idx_template_categories_type ON amro_work_order_template_categories(category_type);

COMMENT ON TABLE amro_work_order_template_categories IS 'Classification system for work package templates';
...
CREATE TABLE IF NOT EXISTS amro_work_order_compliance_records (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    tenant_id UUID NOT NULL,
...
CREATE INDEX idx_wp_compliance_records_tenant ON amro_work_order_compliance_records(tenant_id);
CREATE INDEX idx_wp_compliance_records_wp ON amro_work_order_compliance_records(work_order_id);
CREATE INDEX idx_wp_compliance_records_task ON amro_work_order_compliance_records(task_id);
CREATE INDEX idx_wp_compliance_records_directive ON amro_work_order_compliance_records(directive_id);
CREATE INDEX idx_wp_compliance_records_status ON amro_work_order_compliance_records(compliance_status);
CREATE INDEX idx_wp_compliance_records_certified_by ON amro_work_order_compliance_records(certified_by);

COMMENT ON TABLE amro_work_order_compliance_records IS 'Compliance tracking with digital signatures and evidence for each task';
...
CREATE INDEX idx_crs_work_order ON amro_certificates_release_service(work_order_id);
...
CREATE TABLE IF NOT EXISTS amro_predictive_maintenance_recommendations (
    ...
    work_order_id UUID REFERENCES public.work_orders(id), -- If converted
    reviewed_by UUID REFERENCES auth.users(id),
    reviewed_at TIMESTAMP WITH TIME ZONE,
    review_notes TEXT,
...
CREATE TABLE IF NOT EXISTS amro_work_order_resource_assignments (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    tenant_id UUID NOT NULL,
    work_order_id UUID NOT NULL REFERENCES public.work_orders(id) ON DELETE CASCADE,
...
CREATE INDEX idx_wp_resource_assignments_tenant ON amro_work_order_resource_assignments(tenant_id);
CREATE INDEX idx_wp_resource_assignments_wp ON amro_work_order_resource_assignments(work_order_id);
CREATE INDEX idx_wp_resource_assignments_task ON amro_work_order_resource_assignments(task_id);
CREATE INDEX idx_wp_resource_assignments_resource ON amro_work_order_resource_assignments(resource_id);

COMMENT ON TABLE amro_work_order_resource_assignments IS 'Resource allocation and scheduling for work packages';
...
CREATE TABLE IF NOT EXISTS amro_emergency_work_orders (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    tenant_id UUID NOT NULL,
    work_order_id UUID NOT NULL REFERENCES public.work_orders(id) ON DELETE CASCADE,
...
CREATE INDEX idx_emergency_wp_tenant ON amro_emergency_work_orders(tenant_id);
CREATE INDEX idx_emergency_wp_work_order ON amro_emergency_work_orders(work_order_id);
CREATE INDEX idx_emergency_wp_type ON amro_emergency_work_orders(emergency_type);
CREATE INDEX idx_emergency_wp_urgency ON amro_emergency_work_orders(urgency_level);
CREATE INDEX idx_emergency_wp_status ON amro_emergency_work_orders(declared_at);

COMMENT ON TABLE amro_emergency_work_orders IS 'Emergency/AOG work package tracking with rapid response capabilities';
...
CREATE TABLE IF NOT EXISTS amro_non_scheduled_tasks (
    ...
    converted_to_wp_id UUID REFERENCES public.work_orders(id),
...
CREATE TABLE IF NOT EXISTS amro_work_order_audit_log (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    tenant_id UUID NOT NULL,
    entity_type TEXT NOT NULL CHECK (entity_type IN ('work_order', 'task', 'material', 'compliance', 'certificate', 'resource_assignment')),
...
CREATE INDEX idx_wp_audit_log_tenant ON amro_work_order_audit_log(tenant_id);
CREATE INDEX idx_wp_audit_log_entity ON amro_work_order_audit_log(entity_type, entity_id);
CREATE INDEX idx_wp_audit_log_action ON amro_work_order_audit_log(action);
CREATE INDEX idx_wp_audit_log_performed_at ON amro_work_order_audit_log(performed_at);

COMMENT ON TABLE amro_work_order_audit_log IS 'Immutable audit trail for all work package operations';
...
ALTER TABLE amro_work_order_template_versions ENABLE ROW LEVEL SECURITY;
ALTER TABLE amro_work_order_template_categories ENABLE ROW LEVEL SECURITY;
...
ALTER TABLE amro_work_order_compliance_records ENABLE ROW LEVEL SECURITY;
...
ALTER TABLE amro_work_order_resource_assignments ENABLE ROW LEVEL SECURITY;
ALTER TABLE amro_emergency_work_orders ENABLE ROW LEVEL SECURITY;
...
ALTER TABLE amro_work_order_audit_log ENABLE ROW LEVEL SECURITY;

-- Platform admin access policy for all tables
CREATE POLICY amro_platform_admin_access_template_versions ON amro_work_order_template_versions
    FOR ALL USING (amro_is_platform_admin());

CREATE POLICY amro_platform_admin_access_template_categories ON amro_work_order_template_categories
    FOR ALL USING (amro_is_platform_admin());
...
CREATE POLICY amro_platform_admin_access_wp_compliance_records ON amro_work_order_compliance_records
    FOR ALL USING (amro_is_platform_admin());
...
CREATE POLICY amro_platform_admin_access_wp_resource_assignments ON amro_work_order_resource_assignments
    FOR ALL USING (amro_is_platform_admin());

CREATE POLICY amro_platform_admin_access_emergency_wp ON amro_emergency_work_orders
    FOR ALL USING (amro_is_platform_admin());
...
CREATE POLICY amro_platform_admin_access_audit_log ON amro_work_order_audit_log
    FOR ALL USING (amro_is_platform_admin());

-- Tenant/franchise scope policies (read)
CREATE POLICY amro_tenant_franchise_scope_template_versions_read ON amro_work_order_template_versions
    FOR SELECT USING (
        tenant_id = current_setting('app.current_tenant_id')::UUID
        AND (franchise_id IS NULL OR franchise_id = current_setting('app.current_franchise_id', true)::UUID)
    );

CREATE POLICY amro_tenant_franchise_scope_template_categories_read ON amro_work_order_template_categories
    FOR SELECT USING (
        tenant_id = current_setting('app.current_tenant_id')::UUID
    );
...
CREATE POLICY amro_tenant_franchise_scope_wp_compliance_records_read ON amro_work_order_compliance_records
    FOR SELECT USING (
        tenant_id = current_setting('app.current_tenant_id')::UUID
    );
...
CREATE POLICY amro_tenant_franchise_scope_wp_resource_assignments_read ON amro_work_order_resource_assignments
    FOR SELECT USING (
        tenant_id = current_setting('app.current_tenant_id')::UUID
    );

CREATE POLICY amro_tenant_franchise_scope_emergency_wp_read ON amro_emergency_work_orders
    FOR SELECT USING (
        tenant_id = current_setting('app.current_tenant_id')::UUID
    );
...
CREATE POLICY amro_tenant_franchise_scope_audit_log_insert ON amro_work_order_audit_log
    FOR INSERT WITH CHECK (
        tenant_id = current_setting('app.current_tenant_id')::UUID
    );

CREATE POLICY amro_tenant_franchise_scope_audit_log_read ON amro_work_order_audit_log
    FOR SELECT USING (
        tenant_id = current_setting('app.current_tenant_id')::UUID
    );
...
CREATE INDEX IF NOT EXISTS idx_template_versions_active_effective 
    ON amro_work_order_template_versions(status, effective_from, effective_until)
    WHERE status = 'active';

CREATE INDEX IF NOT EXISTS idx_wp_compliance_by_aircraft_status
    ON amro_work_order_compliance_records(work_order_id, compliance_status);
...
CREATE INDEX IF NOT EXISTS idx_emergency_wp_active
    ON amro_emergency_work_orders(declared_at DESC)
