-- DB-VERIFICATION: amro-rename-work-package-materials-to-work-order-materials-verified
-- DB-ARCH-APPROVAL: pending-amro-arch-board-approval
--
-- Purpose:
-- - Rename public.work_package_materials.work_package_id -> work_order_id
-- - Rename table public.work_package_materials -> public.amro_work_order_materials
-- - Rename associated constraints, indexes, and policies to work-order naming
-- - Repoint any residual inbound FKs referencing work_package_materials(id) to amro_work_order_materials(id)

BEGIN;

DO $$
DECLARE
  v_old_table regclass := to_regclass('public.work_package_materials');
...
BEGIN
  ...
  IF v_old_table IS NOT NULL THEN
    ...
    IF v_has_old_column AND NOT v_has_new_column THEN
      EXECUTE 'ALTER TABLE public.work_package_materials RENAME COLUMN work_package_id TO work_order_id';
    END IF;
  END IF;

  -- Rename table to canonical AMRO work-order naming.
  IF v_old_table IS NOT NULL AND v_new_table IS NULL THEN
    EXECUTE 'ALTER TABLE public.work_package_materials RENAME TO amro_work_order_materials';
  END IF;
  ...
  IF v_has_old_column AND NOT v_has_new_column THEN
    EXECUTE 'ALTER TABLE public.amro_work_order_materials RENAME COLUMN work_package_id TO work_order_id';
  END IF;
  ...
  LOOP
    ...
    v_new_name := replace(v_new_name, 'work_package_materials', 'work_order_materials');
  ...
  v_old_table := to_regclass('public.work_package_materials');
  IF v_old_table IS NOT NULL THEN
    FOR rec IN
      SELECT
        c.conname,
    ...
    LOOP
      v_new_name := replace(rec.conname, 'work_package_materials', 'work_order_materials');
      v_new_name := replace(v_new_name, 'work_package_id', 'work_order_id');
      IF v_new_name = rec.conname THEN
      ...
      v_def := replace(
        replace(rec.condef, 'REFERENCES public.work_package_materials', 'REFERENCES public.amro_work_order_materials'),
        'REFERENCES work_package_materials',
