...
-- DB-ARCH-APPROVAL: Pending architecture board review for additive table + non-breaking FK extension to public.work_orders.

CREATE TABLE IF NOT EXISTS public.work_orders_title (
  ...
  CONSTRAINT pk_work_orders_title PRIMARY KEY (id),
  CONSTRAINT uq_work_orders_title_tenant_title UNIQUE (tenant_id, title),
  CONSTRAINT ck_work_orders_title_wp_title_not_blank CHECK (length(trim(wp_title)) > 0)
);

CREATE INDEX IF NOT EXISTS idx_work_orders_title_tenant_id
  ON public.work_orders_title(tenant_id);

CREATE INDEX IF NOT EXISTS idx_work_orders_title_franchise_id
  ON public.work_orders_title(franchise_id);

ALTER TABLE public.work_orders
  ADD COLUMN IF NOT EXISTS work_order_title_id uuid;
...
BEGIN
  IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'fk_work_orders_work_order_title_id'
      AND conrelid = 'public.work_orders'::regclass
  ) THEN
    ALTER TABLE public.work_orders
      ADD CONSTRAINT fk_work_orders_work_order_title_id
      FOREIGN KEY (work_order_title_id)
      REFERENCES public.work_orders_title(id)
      ON DELETE SET NULL;
  END IF;
END
$$;

CREATE INDEX IF NOT EXISTS idx_work_orders_work_order_title_id
  ON public.work_orders(work_order_title_id);

INSERT INTO public.work_orders_title (tenant_id, franchise_id, title, wp_title)
VALUES
