-- Migration to ensure save_quote_atomic persists cargo_details on quotes.

CREATE OR REPLACE FUNCTION public.save_quote_atomic(p_payload jsonb)
 RETURNS uuid
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  INSERT INTO quotes (
    ...
    billing_address, shipping_address, tenant_id, franchise_id, regulatory_data, cargo_details,
  ...
  VALUES (
    ...
    COALESCE(v_quote_data -> 'cargo_details', '{}'::jsonb),
    ...
    NULL -- current_version_id handled later
  )
    ...
    LOOP
      ...
      IF v_option_id IS NOT NULL THEN
        ...
        INSERT INTO quotation_version_options (
          ...
          COALESCE((v_opt ->> 'margin_percentage')::numeric, 0),
          now(), now()
        )
        ON CONFLICT (id) DO UPDATE SET
          ...
          total_transit_days = EXCLUDED.total_transit_days,
          margin_percentage = EXCLUDED.margin_percentage,
          updated_at = now();
      ELSE
        -- INSERT new option (auto-generated ID)
      ...
      -- Build list of payload leg UUIDs for deletion sync
        ...
        LOOP
          IF (v_leg ->> 'id') IS NOT NULL AND (v_leg ->> 'id') != '' THEN
            ...
            EXCEPTION WHEN invalid_text_representation THEN
              -- Ignore non-UUID temporary client IDs
              NULL;
            END;
          END IF;
        END LOOP;
      ...
      -- Remove stale leg charges/legs when payload removed legs
      IF (v_opt -> 'legs') IS NULL OR jsonb_array_length(v_opt -> 'legs') = 0 THEN
      ...
      ELSIF COALESCE(array_length(v_payload_leg_ids, 1), 0) = 0 THEN
        -- Payload has legs but all are new temp IDs: replace all persisted legs
        DELETE FROM quote_charges
        WHERE quote_option_id = v_option_id
          AND leg_id IS NOT NULL;
      ...
      ELSE
        ...
        WHERE quote_option_id = v_option_id
          AND leg_id IS NOT NULL
          AND leg_id NOT IN (SELECT unnest(v_payload_leg_ids));

        DELETE FROM quotation_version_option_legs
        WHERE quotation_version_option_id = v_option_id
          AND id NOT IN (SELECT unnest(v_payload_leg_ids));
      END IF;
        ...
        LOOP
          ...
          IF v_leg_id IS NOT NULL THEN
             ...
             ON CONFLICT (id) DO UPDATE SET
               ...
               updated_at = now();
          ELSE
             -- Insert new leg (auto-generated ID)
          ...
          IF (v_leg -> 'charges') IS NOT NULL AND jsonb_array_length(v_leg -> 'charges') > 0 THEN
             ...
             LOOP
               ...
               -- Resolve Basis ID (ID first, then Code)
               v_basis_id := NULL;
               IF (v_charge ->> 'basis_id') IS NOT NULL THEN
                 v_basis_id := (v_charge ->> 'basis_id')::uuid;
               END IF;
               
               IF v_basis_id IS NULL THEN
                 v_basis_code := v_charge ->> 'basis';
                 SELECT id INTO v_basis_id FROM charge_bases WHERE code = v_basis_code LIMIT 1;
               END IF;

               -- Resolve Currency ID (ID first, then Code)
               v_currency_id := NULL;
               IF (v_charge ->> 'currency_id') IS NOT NULL THEN
                 v_currency_id := (v_charge ->> 'currency_id')::uuid;
               END IF;
      ...
      -- Always replace combined charges to prevent stale/global duplicates
      DELETE FROM quote_charges
      WHERE quote_option_id = v_option_id
        AND leg_id IS NULL;

      -- Process Combined Charges (Global for Option)
