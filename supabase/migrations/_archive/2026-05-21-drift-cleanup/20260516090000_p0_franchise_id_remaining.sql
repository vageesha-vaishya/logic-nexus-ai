-- ══════════════════════════════════════════════════════════════════════════
-- P0 Hierarchy Fix (follow-up): franchise_id NOT NULL — remaining gaps
-- ══════════════════════════════════════════════════════════════════════════
--
-- Covers tables skipped or not reached by 20260515210000_p0_hierarchy_franchise_id.sql:
--
--   public.compliance_domain_verifications   (0 rows, has tenant_id)
--   public.compliance_legal_holds            (0 rows, has tenant_id)
--   public.compliance_retention_policies     (0 rows, has tenant_id)
--   public.compliance_screenings             (2 rows, 0 nulls — just enforce NOT NULL)
--   public.compliance_records_duplicate      (2546 rows, 1440 null franchise_ids — backfill)
--   module_finance.module_finance_invoices   (16 rows, franchise_id nullable — backfill)
--   module_finance.module_finance_payments   (0 rows, franchise_id nullable — enforce NOT NULL)
--   module_finance.module_finance_invoice_items (20 rows, franchise_id missing — add + backfill)
--
-- Deliberately skipped (no tenant_id anchor — separate epic):
--   public.compliance_checks
--   public.compliance_actions_log
--
-- DB-VERIFICATION: p0-franchise-id-remaining-v1
...
-- compliance_domain_verifications (0 rows, has tenant_id)
...
-- compliance_legal_holds (0 rows, has tenant_id)
...
-- compliance_retention_policies (0 rows, has tenant_id)
...
-- compliance_screenings (2 rows, all franchise_ids already set)
ALTER TABLE public.compliance_screenings
  ALTER COLUMN franchise_id SET NOT NULL;

-- compliance_records_duplicate (2546 rows, 1440 null franchise_ids)
...
-- module_finance_invoices (16 rows, franchise_id nullable)
...
-- module_finance_payments (0 rows, franchise_id nullable)
...
-- module_finance_invoice_items (20 rows, missing franchise_id — inherit from parent invoice)
...
COMMENT ON COLUMN public.compliance_records_duplicate.franchise_id IS
  'NOT NULL since 2026-05-16 P0 follow-up. Null rows backfilled to tenant primary franchise. Table is a legacy artifact — no production code writes to it.';
