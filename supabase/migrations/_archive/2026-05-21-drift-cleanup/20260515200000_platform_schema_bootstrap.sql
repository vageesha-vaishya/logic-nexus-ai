...
-- ══════════════════════════════════════════════════════════════════════════
--
-- Adds the missing platform infrastructure tables to the existing
-- platform schema (which already has llm_usage + llm_provider_configs).
--
-- New tables:
--   platform.audit_log               — append-only mutation log (ADR-005)
--   platform.access_log              — append-only access decision log (ADR-014)
--   platform.idempotency_keys        — webhook / API dedup
--   platform.integrations            — integration registry (ADR-015)
--   platform.integration_credentials — vault-backed credential refs (ADR-016)
--   platform.integration_log         — outbound/inbound call log (ADR-018)
--   platform.webhook_subscriptions   — outbound webhook subscriptions (ADR-019)
--   platform.integration_dlq         — dead-letter queue
--   platform.service_accounts        — non-human API identities
--
-- Schema augmentations:
--   public.franchises   + parent_franchise_id, path ltree (multi-level hierarchy)
--   public.user_roles   + domain, scope_paths ltree[], expires_at, conditions, granted_by
--
-- New SECURITY DEFINER helper functions in platform:
--   platform.is_platform_admin(uid)
--   platform.get_user_tenant_id(uid)
--   platform.get_user_franchise_id(uid)
--   platform.user_can_access_franchise(uid, franchise_id)
--   platform.is_within_tenant_subtree(uid, tenant_id)
--   platform.user_has_scope(uid, scope)
--
...
-- ── 3. platform.audit_log — append-only mutation log ───────────────────────
...
-- Users see their own audit events; acted_by for impersonation double-tag
CREATE POLICY audit_log_own_select ON platform.audit_log
  FOR SELECT USING (user_id = auth.uid() OR acted_by = auth.uid());

-- ── 4. platform.access_log — append-only access decision log ───────────────
...
-- ── 5. platform.idempotency_keys — webhook / API dedup ─────────────────────
...
-- ── 6. platform.integrations — integration registry ────────────────────────
...
CREATE INDEX IF NOT EXISTS idx_integrations_lifecycle
  ON platform.integrations (lifecycle_state)
...
-- Only service-role can access credentials — anon/JWT clients never see vault refs
...
-- ── 8. platform.integration_log — call log ─────────────────────────────────
...
CREATE INDEX IF NOT EXISTS idx_webhook_subs_status
  ON platform.webhook_subscriptions (status) WHERE status = 'active';
...
-- ── 10. platform.integration_dlq — dead-letter queue ───────────────────────
...
CREATE INDEX IF NOT EXISTS idx_service_accounts_active
  ON platform.service_accounts (is_active, expires_at);
...
-- ── 12. RLS helper functions (SECURITY DEFINER, search_path hardened) ──────

-- platform.is_platform_admin: true if uid has global platform_admin role
...
-- platform.get_user_tenant_id: primary tenant for a user
...
-- platform.get_user_franchise_id: primary franchise for a user
...
-- platform.is_within_tenant_subtree: uid belongs to tenant (platform_admin always true)
...
-- platform.user_can_access_franchise: direct + LTREE subtree + tenant_admin
...
AS $$
  SELECT
    platform.is_platform_admin(uid)
    -- Tenant admin has access to all franchises in their tenant
    OR EXISTS (
      SELECT 1 FROM public.user_roles ur
    ...
    -- LTREE subtree: user's franchise is an ancestor of target_franchise_id
    OR EXISTS (
      SELECT 1
      ...
      WHERE ur.user_id = uid
        ...
        AND tf.path <@ uf.path     -- tf is a descendant of uf
        AND (ur.expires_at IS NULL OR ur.expires_at > now())
    )
$$;

-- platform.user_has_scope: ABAC scope check (platform_admin always passes)
...
-- ── 14. Seed platform-level integrations for current 3rd-party services ─────
-- Registers existing external dependencies in the registry (ADR-015)
