...
END;
$$ LANGUAGE plpgsql;
-- 4. Re-attach Trigger (just to be safe, though function update is usually enough if trigger points to function name)
-- No need to drop/create trigger if it calls the same function name, but explicit is fine.
