-- DB-VERIFICATION: aircraft-template-template-name-unique-constraint-reviewed
...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION 'Column public.aircraft_template.template_name does not exist.';
  END IF;
  ...
  SELECT EXISTS (
    ...
    WHERE n.nspname = 'public'
      ...
      AND c.contype = 'u'
      AND pg_get_constraintdef(c.oid) = 'UNIQUE (template_name)'
  ...
  IF duplicate_count > 0 THEN
    RAISE EXCEPTION
      'Cannot create unique constraint uq_aircraft_template_template_name: % duplicate template_name groups exist.',
      duplicate_count;
  END IF;
...
COMMENT ON CONSTRAINT uq_aircraft_template_template_name ON public.aircraft_template IS
  'Enforces unique template_name values for aircraft templates.';
