  ...
  IF v_options IS NOT NULL AND jsonb_array_length(v_options) > 0 THEN
    ...
    LOOP
      IF (v_opt ->> 'id') IS NOT NULL THEN
        ...
        -- Delete legs not in payload (optional, if we want to remove deleted legs)
        -- DELETE FROM quotation_legs WHERE option_id = v_option_id AND NOT (id = ANY(v_payload_leg_ids));
        ...
        IF (v_opt -> 'legs') IS NOT NULL AND jsonb_array_length(v_opt -> 'legs') > 0 THEN
          ...
          LOOP
            ...
            IF (v_leg ->> 'id') IS NOT NULL THEN
               ...
               UPDATE quotation_legs
               SET 
                 sequence_number = v_sort_order,
                 ...
                 transit_time_days = (v_leg ->> 'transit_time_days')::int,
                 start_date = (v_leg ->> 'start_date')::date,
                 end_date = (v_leg ->> 'end_date')::date,
                 updated_at = now()
               WHERE id = v_leg_id;
            ELSE
               v_leg_id := gen_random_uuid();
               INSERT INTO quotation_legs (
                 id, option_id, tenant_id, sequence_number,
                 transport_mode, origin_location_id, destination_location_id,
                 carrier_id, transit_time_days, start_date, end_date,
                 created_at, updated_at
               ) VALUES (
                 ...
                 (v_leg ->> 'transit_time_days')::int,
                 (v_leg ->> 'start_date')::date,
                 (v_leg ->> 'end_date')::date,
                 now(), now()
               );
            END IF;
            ...
            IF (v_leg -> 'charges') IS NOT NULL AND jsonb_array_length(v_leg -> 'charges') > 0 THEN
               v_charge_sort := 0;
               -- Delete existing charges for this leg? Or upsert?
               -- Safe strategy: Delete all charges for this leg and re-insert
               DELETE FROM quotation_charges WHERE leg_id = v_leg_id;
               ...
               LOOP
                 ...
                 v_rate := (v_charge ->> 'rate')::numeric;
                 v_amount := (v_charge ->> 'amount')::numeric;

                 v_charge_side := v_charge ->> 'side'; -- 'sell' or 'buy'
      ...
      ELSE
        -- Create new option logic (omitted for brevity, focus on upsert)
        NULL;
      END IF;
    END LOOP;
  END IF;
