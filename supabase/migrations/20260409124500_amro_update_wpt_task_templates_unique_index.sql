...
BEGIN
  IF to_regclass('public.work_package_template_task_templates') IS NULL THEN
    RAISE EXCEPTION 'Table public.work_package_template_task_templates does not exist';
  END IF;
  ...
  WITH ranked_rows AS (
    ...
    FROM public.work_package_template_task_templates
  ),
  duplicate_rows AS (
    SELECT id
    FROM ranked_rows
  ...
  DELETE FROM public.work_package_template_task_templates t
  USING duplicate_rows d
  WHERE t.id = d.id;

  GET DIAGNOSTICS deleted_count = ROW_COUNT;
  RAISE NOTICE 'Removed % duplicate work_package_template_task_templates rows before unique index rebuild', deleted_count;
END
$migration$;
