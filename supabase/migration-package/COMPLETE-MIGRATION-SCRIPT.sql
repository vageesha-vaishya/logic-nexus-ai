...
BEGIN
  IF NOT EXISTS (
    ...
    CREATE TYPE public.shipment_type AS ENUM ('air','ocean','ground','rail','courier','ocean_freight','air_freight','inland_trucking','railway_transport','movers_packers');
  ELSE
    FOREACH lbl IN ARRAY ARRAY['air','ocean','ground','rail','courier','ocean_freight','air_freight','inland_trucking','railway_transport','movers_packers'] LOOP
      IF NOT EXISTS (
        SELECT 1 FROM pg_enum e JOIN pg_type t ON t.oid = e.enumtypid JOIN pg_namespace n ON n.oid = t.typnamespace
