...
3. Set `SITE_URL=https://app.sosservices.online` and `ADDITIONAL_REDIRECT_URLS=https://app.sosservices.online/**` (GoTrue reads the latter as `GOTRUE_URI_ALLOW_LIST`; the `**` glob covers `/`, `/auth`, and `/auth/callback` in one entry).
...
6. Set `site_url` and `uri_allow_list` to the same values as step 3.
