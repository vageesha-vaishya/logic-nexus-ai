...
- AI-smart-mode quotes don't have this problem: the **deployed** `ai-advisor` Edge Function's own prompt (`supabase/functions/ai-advisor/index.ts:298-309` — not the client-side `src/hooks/useAiAdvisor.ts`, which only holds a thinner fallback prompt used when Supabase itself is unreachable) explicitly mandates real per-leg charges: *"Break down each route into specific legs (Pickup -> Port -> Main Leg -> Port -> Delivery)"* and *"You MUST calculate and populate charges for **EVERY** leg."* `mapOptionToQuote` already handles that shape correctly (`normalized.legs[].charges`, read at `quote-mapper.ts:202-219` and `:289+`).
...
- Not touching `useAiAdvisor.ts`'s mock-fallback data, `generateSimulatedRates` (`src/lib/simulation-engine.ts` — a separate, purely-client-side simulation engine, distinct from `rate-engine`'s own server-side simulation path this spec does touch; triggered when a `rate-engine` call for a given combo errors out or returns zero options, not only on outright unreachability), or the two Edge Function bugs found while debugging the map feature (invalid OpenAI key, unimplemented `validate_carrier_service_availability` action) — those are tracked separately.
- Not retroactive: quotes already saved before this fix ships keep whatever charge data was persisted at save time (one generic line, if that's what they had). `FinalizeSection.tsx`'s `useChargesManager` seeds its editable charge state from `selectedOption.legs` only when no saved draft exists (`initialLegs ?? selectedOption.legs`, `useChargesManager.ts:247`) — a reloaded draft uses its own persisted `legs`, bypassing this fix entirely. Only newly-generated quotes benefit. (Verified directly, not assumed — confirms `FinalizeSection.tsx` needs no code change per §4, but the "no change needed" reasoning only holds for the new-quote path.)
