...
3. **Sequences correct:** for every sequence in all 20 schemas, self-hosted's value is `>=` production's captured value. This is the check that prevents the primary-key collision described in §1a.3.
