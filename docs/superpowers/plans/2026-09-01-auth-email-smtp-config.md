...
**Use `/auth/v1/verify` with a deliberately invalid token, not `/recover`.** Both endpoints funnel the redirect target through the same `utilities.GetReferrer` → `getRedirectTo` (query, then post data, then `Referer`, then `SiteURL`) → `isRedirectURLValid` path, so this probe — sending `redirect_to` as a query parameter, which is honoured — exercises the identical extraction and validation logic `/recover` uses, and 302s to the result immediately: no email sent, no token consumed, decision readable straight off the `Location` header. **What it does not reach is the final hop:** `/recover`'s mailer templating the resolved referrer into the emailed link body. That plumbing is separately evidenced only by the real delivered emails in Task 1 Step 7 and Task 2 Step 5, not by this probe. (An earlier version of this step POSTed `{"options":{"redirectTo": ...}}` to `/recover`. That is the **supabase-js client** shape, which the library translates before it reaches the API; GoTrue reads `redirect_to` from query or post data, then `Referer`, then `SiteURL`, so the value was silently ignored and the check falsely appeared to fail.)
