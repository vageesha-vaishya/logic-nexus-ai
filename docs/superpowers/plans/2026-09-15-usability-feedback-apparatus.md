...
DECLARE
  ...
  v_count integer;
BEGIN
  -- Fixture: two tenants, a regular user in each, one tenant_admin in tenant A.
  INSERT INTO public.tenants (id, name) VALUES (gen_random_uuid(), '[smoke_test] UX FB Tenant A') RETURNING id INTO v_tenant_a;
  INSERT INTO public.tenants (id, name) VALUES (gen_random_uuid(), '[smoke_test] UX FB Tenant B') RETURNING id INTO v_tenant_b;
  ...
  -- Cleanup (service-role context restored by RESET ROLE above).
  DELETE FROM public.ux_feedback WHERE tenant_id IN (v_tenant_a, v_tenant_b);
  DELETE FROM public.user_roles WHERE user_id IN (v_user_a, v_user_b, v_admin_a);
