...
**Side finding, not fixed:** while confirming `pg_cron` itself works (it does — a throwaway job was scheduled and unscheduled to prove it), `core.outbox`'s own cron job was found to be absent from `cron.job` on production, despite its migration explicitly scheduling one. Likely explanation: `cron.job` is extension-internal operational state that didn't survive this database's reconstitution from prod (a recurring theme elsewhere in this audit's findings), not a new regression. `llm-usage-partition-provisioner`'s entry was confirmed present and `active` immediately after this fix, but the same non-survival risk applies to it across any future reconstitution and should be checked for, not assumed fixed permanently by this migration alone.
