...
**Live browser test of the fix (2026-09-10) surfaced a second, more serious bug: the `PAID_FALLBACK_ON_FAILURE` entry just added for `markets.research_thread` was dead code.** Sending a message through the actual Research Thread UI (`/dashboard/markets/research`) failed outright with "Edge Function returned a non-2xx status code." Server logs showed why: this tenant's tenant-wide-default provider config (a `gemini` row with no `domain` set, `is_default: true`) has a missing vault secret (`"tenant config found but api_key missing in vault"`), so `resolveConfig` correctly skipped it and fell through to env-based routing — which, with no `ANTHROPIC_API_KEY` set in this self-hosted deployment, landed on `VLLM_BASE_URL`/`VLLM_API_KEY` — the *other*, already-documented dead rig (`vllm.sosservices.online`, distinct from the real `portal.sosservices.online` rig — see the vLLM rig clarification earlier in this project) — which correctly 401'd. That part is all working as designed. What wasn't: **`markets.research_thread`'s new fallback entry never fired**, because `markets-research/index.ts` (its only caller, anywhere) calls `callLLMConversation`, a separate function from `callLLM` — and `callLLMConversation` never had `callLLM`'s primary-then-paid-fallback retry logic at all. It always existed as a plain resolve-and-call-once path, so *every* task ever routed through it (today, only `research_thread`) would fail outright on any primary-provider error, fallback entry or not.

**Fixed and deployed, `0ef5bc16`:** extracted `callLLMConversation`'s provider-dispatch switch into an `attempt(cfg)` helper and wrapped it with the exact same primary-config-then-paid-fallback retry `callLLM` already uses (try the resolved config first, unconditionally per policy; on failure, retry once against `PAID_FALLBACK_ON_FAILURE[taskId]` if one exists and its env credential is set). Lint clean, exhaustiveness and brace/paren balance re-checked, no stray unconverted variable references (mechanical `config` → `cfg` rename across the extracted closure, verified by grep). Deployed via the same host bind-mount path, container came back healthy.

**Verified end-to-end, live in the browser, no synthetic test:** resent the exact same message through the real Research Thread UI. It now succeeds and renders a real, correctly-grounded answer (the model declined to fabricate banking-sector claims with no data in the thread's context and asked for source data instead — its system prompt's grounding rule working as intended, not a bug). `platform.llm_usage` shows the complete, policy-correct sequence:

| id | provider | model | status | latency | notes |
|---|---|---|---|---|---|
| 3832 | `local-qwen` | `qwen3.8-27b-awq` | `error` (`provider_401`) | 1706ms | primary attempt, pre-fix — request failed outright here, no fallback row |
| 3833 | `local-qwen` | `qwen3.8-27b-awq` | `error` (`provider_401`) | 149ms | primary attempt, post-fix — same expected failure (stale rig, unrelated to this fix) |
