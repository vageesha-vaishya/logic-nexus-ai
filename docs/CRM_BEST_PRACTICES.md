...
* **Cleanup Strategy:** Automated cleanup scripts identify and consolidate "Legacy" data (e.g., merging 'courier' and 'courier service' into 'courier\_express').

## 4. UI/UX Framework: "Dynamic Attribute Rendering"

We implemented a **Metadata-Driven UI** pattern (`ServiceAttributeRenderer`).

* **How it works:** The UI asks the database: "What attributes does 'Ocean Freight' have?" The DB responds with metadata (Key: 'container\_type', Type: 'select', Options: \['20GP', '40HC']). The UI dynamically builds the form.
