...
### 9.9 Real tool-fetched rates weren't reconciled against the model's final option (found auditing §10 item 12, fixed pre-emptively)

Following up on the Smart Quote result-integrity audit (§10 item 12), the same class of bug was checked against this tool-calling framework specifically, since it's the one place a genuinely real, externally-verified number (a live SeaRates rate) meets the model's own free-form final JSON generation. Confirmed: nothing did. `executeRateToolCall`'s real `NormalizedRate` was handed to the model as Round 2 context, but nothing then checked that the model's *final* option for that carrier actually used it — the model was free to restate a different price, the same "carrier ONE shows two different prices in the same response" failure mode from the original audit, except here one of the two numbers could be a real, paid third-party quote being silently overridden.

**Not a live bug today** — §9.6 confirms zero tenants have a real adapter configured, so `realRatesFetched` is always empty and this reconciliation is currently a no-op. Fixed anyway, before the first real adapter goes live rather than after: `generateSmartQuotes` now collects every successful `NormalizedRate` the tool call executor returns into `realRatesFetched`, and `applyDynamicPricing` matches it against each final option by carrier name (case-insensitive — `NormalizedRate` has no option id the model could echo back, and carrier name is the only field both sides share). On a match, the option's `base_fare` and `transit_time.total_days` are overwritten with the real, tool-fetched figures — trusting the external, verified number over the model's own restatement, the same principle already applied to carrier name and legs/price_breakdown internal consistency, now extended past the model's own output to a genuinely external one. The real rate's own itemized surcharges are preserved as separate `provider_*`-labeled leg charges rather than folded into `base_fare`, per `NormalizedRate`'s own contract that surcharges are never pre-summed by an adapter.
...
12. ~~Smart Quote result-integrity audit (simulated rate mislabeling, reliability defaulting to 0, unreconciled cheapest/best_value tags, ungrounded CO2 estimates)~~ **Done.** Triggered by the user auditing a real Shanghai→Norfolk result and asking "can you believe this shipment has an actual quote to ship at this cost and price" — four distinct, confirmed problems, all fixed:
    ...
    Follow-up audit ("check the other rate providers for the same issues") found the same bug classes in two more places, both fixed the same way:
    - **`QuoteResultsList.tsx`** (the second, independent option-card renderer used by `UnifiedQuoteComposer`, parallel to `SmartQuoteRateCard.tsx`) had its own `OptionSourceBadge` with no `is_simulated` check at all — a simulated option here rendered "Market Rate" with a tooltip claiming *"Real-time market rate verified via Carrier API,"* a stronger false claim than the bug already fixed. Fixed with the same `is_simulated` branch, labeled "Estimated Rate." Its three separate CO2 badge conditions also had the same object-presence-instead-of-string-truthiness bug as `SmartQuoteRateCard.tsx`'s pre-fix condition (`option.co2_kg || option.environmental` instead of checking `.co2_emissions`), rendering an empty CO2 pill once `co2_emissions` started being correctly blanked to `""`; fixed identically at all three sites. (Reliability-0 and tier-reconciliation did **not** need a separate fix here — both are fixed upstream in `quote-mapper.ts`/`useRateFetching.ts`, which this component already consumes, so it inherited them automatically.)
    - **The §9 third-party rate-provider tool-calling framework** — see the new §9.9 for the finding and fix (a real tool-fetched rate wasn't reconciled against the model's final option, the same "two independent numbers, nothing forces them to agree" bug, just extended to an externally-verified number instead of an internal one). Not a live bug today (§9.6: zero tenants have a real adapter configured), fixed pre-emptively before that changes.
