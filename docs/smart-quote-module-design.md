...
11. **`LocationAutocomplete` only sets `*Details` on an explicit dropdown click, not on an auto-matched typed value** (found verifying item 1's fix, §7.1's closing note). Typing a destination that happens to render a match without the user clicking a suggestion leaves `destinationDetails` `null` for that field on that request, silently falling back to the fragile string-match §7.1 just fixed. A frontend fix, out of scope for the `rate-engine`/`ai-advisor` changes made here.
