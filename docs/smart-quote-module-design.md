...
### 7.1 What "MARKET RATE" actually is today — an important caveat

`rate-engine` computes carrier options and the frontend renders them alongside the AI options, labeled "MARKET RATE" vs. "AI GENERATED." The label implies these are always real, DB-backed carrier pricing. **They frequently are not.** `rate-engine`'s location resolution (`resolveLocation()`) does an *exact string match* on `ports_locations.location_code` against whatever string the frontend sent as `origin`/`destination` — but the Smart Quote page sends the location's **display name** (e.g. `"Nhava Sheva"`), not its code (e.g. `"INNSA"`), in that field. When the match fails (the common case), `originId`/`destId` come back `null`, the real `carrier_rates` query is skipped entirely, and `rate-engine` falls through to its "10+ options guarantee" fallback — which fabricates one option per carrier in a hardcoded list, at `basePrice * (0.85 + Math.random() * 0.3)`, i.e. **a random number within ±15% of a hardcoded base rate**, not a real quote. Nothing in `rate-engine`'s response distinguishes a real row from a simulated one except the simulated ones' `sim_`-prefixed `id`, which the frontend doesn't check and doesn't surface. **This means most "MARKET RATE" options shown in the UI today are random, not real, whenever the free-text origin/destination string doesn't happen to equal a `location_code` exactly** — worth fixing in `rate-engine` itself as a follow-up (not done here — out of scope for a Smart Quote/`ai-advisor` change), but critical to know before treating `rate-engine`'s output as a trustworthy benchmark input.

This is exactly why §7.2 below does **not** call `rate-engine`'s HTTP endpoint and try to filter its response for the real rows — it queries `carrier_rates` directly from `ai-advisor`, using a reliable UUID rather than the same fragile string match, so it only ever sees genuine rows.
...
1. ~~Wire `rate-engine`'s live market-rate average into `ai-advisor`'s benchmark context.~~ **Done — §7.2.** Built by querying `carrier_rates` directly, not through `rate-engine`'s HTTP endpoint. Along the way this surfaced a real, pre-existing bug worth its own fix: `rate-engine`'s own `resolveLocation()` exact-matches the free-text `origin`/`destination` string against `ports_locations.location_code`, but the Smart Quote page sends the display *name*, not the code — so on the common case the match fails silently, `carrier_rates` is never queried, and `rate-engine`'s response is filled entirely with randomly-simulated "MARKET RATE" options with nothing indicating to the frontend or user that they aren't real (§7.1). Not fixed here (out of scope for an `ai-advisor` change) — the fix is the same one §7.2 used: read `originDetails.id`/`destinationDetails.id`, a `ports_locations` UUID the frontend already sends, instead of re-resolving a free-text string.
