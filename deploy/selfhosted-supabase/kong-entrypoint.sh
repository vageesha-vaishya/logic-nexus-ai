...
#
# Copied verbatim from upstream supabase/supabase:docker/volumes/api/kong-entrypoint.sh
# (fetched 2026-08-22 from the supabase/supabase repo, docker/ sparse-checkout,
# alongside kong/kong:3.9.3's docker-compose.kong.yml override). Do not paraphrase
# or hand-roll this logic — it is the only thing that turns the $SUPABASE_ANON_KEY /
# $SUPABASE_SERVICE_KEY placeholders in kong.yml into real values before Kong starts.
# If this script is skipped, Kong authenticates requests against the literal string
# "$SUPABASE_ANON_KEY" instead of the real key, and every API call fails auth silently.
