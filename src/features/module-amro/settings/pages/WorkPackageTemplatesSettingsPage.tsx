...
interface WorkPackageTemplateFormData {
  template_code: string;
  template_name: string;
  description: string;
  maintenance_type: string;
...
const DEFAULT_FORM_DATA: WorkPackageTemplateFormData = {
  template_code: '',
  template_name: '',
  description: '',
...
export function WorkPackageTemplatesSettingsPage() {
  ...
  const [formData, setFormData] = useState<WorkPackageTemplateFormData>(DEFAULT_FORM_DATA);
  const [formLoading, setFormLoading] = useState(false);
  const [formErrors, setFormErrors] = useState<Partial<Record<keyof WorkPackageTemplateFormData, string>>>({});
  ...
  const openEditForm = (template: WorkPackageTemplate) => {
  ...
  const validateForm = (): boolean => {
    const errors: Partial<Record<keyof WorkPackageTemplateFormData, string>> = {};
  ...
  const columns: Column<WorkPackageTemplate>[] = [
    {
      key: 'template_code',
      label: 'Code',
      sortable: true,
  ...
  const getActions = (row: WorkPackageTemplate) => AmroActions.crud({
