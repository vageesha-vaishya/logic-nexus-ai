...
import { WorkOrderTemplateCreateSection } from './WorkOrderTemplateCreateSection';
...
describe('WorkOrderTemplateCreateSection', () => {
  beforeEach(() => {
    vi.stubGlobal('fetch', vi.fn(async (input: RequestInfo | URL) => {
      const url = String(input);
      if (url.includes('/api/v2/amro/work-order-templates/model-options')) {
        return {
          ok: true,
          status: 200,
          json: async () => ({
      ...
      if (url.includes('/api/v2/amro/work-order-templates/task-template-options')) {
