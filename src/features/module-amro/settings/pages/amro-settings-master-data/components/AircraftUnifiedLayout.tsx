...
export function AircraftUnifiedLayout({
  ...
  const hasVisibleNode = (node: ReactNode): boolean => {
    ...
    if (Array.isArray(node)) {
      return node.some((item) => hasVisibleNode(item));
    }
    if (isValidElement(node)) {
      if (node.type === Fragment) {
        const fragmentChildren = (node.props as { children?: ReactNode } | null)?.children;
        return hasVisibleNode(fragmentChildren ?? null);
      }
      return true;
    }
    return false;
  };
  const showHeader = showHeaderSummary || showNavRail || showSearchControls || showResultSummary;
  const hasBodyContent = loading || Boolean(error) || hasVisibleNode(children);
  if (!showHeader && !hasBodyContent) {
    return null;
  }
