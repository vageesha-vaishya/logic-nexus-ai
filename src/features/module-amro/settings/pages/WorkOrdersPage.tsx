...
async function fetchWorkOrders(
  ...
  return {
    workOrders: Array.isArray(records) ? records : [],
    total: json.output?.total || json.count || (Array.isArray(records) ? records.length : 0),
  };
}

async function createWorkOrder(accessToken: string, tenantId: string, data: WorkOrderFormData): Promise<WorkOrder> {
...
async function updateWorkOrder(accessToken: string, id: string, data: Partial<WorkOrderFormData>): Promise<WorkOrder> {
...
async function deleteWorkOrder(accessToken: string, id: string): Promise<void> {
  const response = await fetch(`/api/v2/amro/master-data/work_orders/${id}`, {
    method: 'DELETE',
    headers: { Authorization: `Bearer ${accessToken}` },
  });
...
export function WorkOrdersPage() {
  ...
  const [formData, setFormData] = useState<WorkOrderFormData>(DEFAULT_FORM_DATA);
  const [formLoading, setFormLoading] = useState(false);
  const [formErrors, setFormErrors] = useState<Partial<Record<keyof WorkOrderFormData, string>>>({});

  // Delete state
  const [deleteConfirmOpen, setDeleteConfirmOpen] = useState(false);
  const [deleteCandidate, setDeleteCandidate] = useState<WorkOrder | null>(null);
  ...
  const loadWorkOrders = useCallback(async () => {
    ...
    try {
      const result = await fetchWorkOrders(accessToken, tenantId, {
  ...
  const openEditForm = (wp: WorkOrder) => {
    setEditingWorkOrder(wp);
    setFormData({
  ...
  const validateForm = (): boolean => {
    ...
    if (!formData.work_order_number.trim()) errors.work_order_number = 'Work package number is required';
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (editingWorkOrder) {
        await updateWorkOrder(accessToken, editingWorkOrder.id, formData);
        toast.success('Work package updated');
      } else {
        await createWorkOrder(accessToken, tenantId, formData);
  ...
  const handleDelete = (wp: WorkOrder) => {
  ...
  const confirmDelete = async () => {
    ...
    try {
      await deleteWorkOrder(accessToken, deleteCandidate.id);
  ...
  const columns: Column<WorkOrder>[] = [
    {
      ...
      sortable: true,
      width: 'w-36',
      render: (row) => <Badge variant="outline" className="font-mono">{row.work_order_number}</Badge>,
    },
    {
      key: 'title',
      label: 'Title',
  ...
  const getActions = (row: WorkOrder) => AmroActions.crud({
  ...
  return (
    <AmroUnifiedPageLayout
      ...
      kpiMetrics={[
        ...
        { label: 'In Progress', value: workOrders.filter(wp => wp.status === 'in_progress').length, icon: <CheckSquare className="h-4 w-4" /> },
      ]}
    >
      <AmroUnifiedTable
        columns={columns}
      ...
      <AmroUnifiedForm
        ...
        title={editingWorkOrder ? 'Edit Work Package' : 'Create Work Package'}
        description={editingWorkOrder ? 'Update work package details' : 'Add a new work package'}
        onSubmit={handleSubmit}
        onCancel={() => setFormOpen(false)}
        loading={formLoading}
        submitLabel={editingWorkOrder ? 'Update' : 'Create'}
      >
        <AmroUnifiedForm.Tabs defaultValue="basic">
          <AmroUnifiedForm.Tab value="basic" label="Basic Info">
            <AmroUnifiedForm.Section title="Work Package Details">
              <AmroUnifiedForm.Field label="WP Number" required error={formErrors.work_order_number}>
                <Input
      ...
      <AmroUnifiedForm
        ...
        description={`Are you sure you want to delete work package "${deleteCandidate?.work_order_number}"?`}
