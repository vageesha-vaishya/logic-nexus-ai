...
export function AmroWorkOrderCreateWizard({
  ...
  const { options: templateOptions } = useWorkOrderTemplateOptions(open);
  ...
  return (
    <AmroRecordWizard
      mode="create"
      steps={steps}
      initialData={getWorkOrderWizardInitialData(preselectedAircraftId)}
      ...
      onSubmit={async (formData) => {
        ...
        if (creationPath === 'emergency') {
          ...
          const workOrderTitleId = UUID_PATTERN.test(selectedTitleId) ? selectedTitleId : undefined;
          await createWPMutation.mutateAsync({
            aircraft_id: String(formData.aircraft_id || '').trim(),
            title: selectedTitle?.title || undefined,
            work_order_title_id: workOrderTitleId,
            ...
            priority: Number(formData.priority || 3) as WorkOrderPriority,
