...
): Promise<{ templates: WorkPackageTemplate[]; total: number }> {
...
export function AmroWorkPackageTemplatesPage() {
  ...
  const [editTemplate, setEditTemplate] = useState<WorkPackageTemplate | null>(null);
  const [previewTemplate, setPreviewTemplate] = useState<WorkPackageTemplate | null>(null);
  const [versionManagerOpen, setVersionManagerOpen] = useState(false);
  const [versionTemplate, setVersionTemplate] = useState<WorkPackageTemplate | null>(null);
  const [cloneDialogOpen, setCloneDialogOpen] = useState(false);
  const [cloneTemplate, setCloneTemplate] = useState<WorkPackageTemplate | null>(null);
  const [deleteConfirmOpen, setDeleteConfirmOpen] = useState(false);
  const [deleteCandidate, setDeleteCandidate] = useState<WorkPackageTemplate | null>(null);
  ...
  const sortedTemplates = useMemo(() => {
    ...
    sorted.sort((a, b) => {
      ...
      const aVal = a[field as keyof WorkPackageTemplate];
      const bVal = b[field as keyof WorkPackageTemplate];
  ...
  const handleDelete = (template: WorkPackageTemplate) => {
  ...
  const handleClone = (template: WorkPackageTemplate) => {
    setCloneTemplate(template);
    setCloneDialogOpen(true);
  };

  const handleManageVersions = (template: WorkPackageTemplate) => {
    setVersionTemplate(template);
    setVersionManagerOpen(true);
  };

  const handlePreview = (template: WorkPackageTemplate) => {
