...
import { useAmroWorkOrderViewMutations } from './useAmroWorkOrderViewMutations';
import type {
...
export function useAmroWorkspaceState() {
  ...
  // Phase 8f.4i — final 2 inline mutators carved into useAmroWorkOrderViewMutations.
  const {
    saveCurrentWorkOrderView,
    deleteSelectedWorkOrder,
  } = useAmroWorkOrderViewMutations({
    apiBaseUrl,
    authHeaders,
    isApiTemporarilyUnavailable,
    markApiTemporarilyUnavailable,
