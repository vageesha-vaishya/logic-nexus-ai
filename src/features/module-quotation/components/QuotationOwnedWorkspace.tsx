        ...
        <CardContent className="space-y-3">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <Badge variant={state.pluginValidation.isValid ? 'secondary' : 'destructive'}>
              {state.pluginValidation.isValid ? 'Plugin Validation Ready' : 'Plugin Validation Blocked'}
            </Badge>
            <Badge variant={state.canAccept ? 'secondary' : 'destructive'}>
              {state.canAccept ? 'Acceptance Gate Ready' : 'Acceptance Gate Blocked'}
            </Badge>
          </div>
        </CardContent>
      </Card>
      <Accordion type="single" collapsible className="w-full" defaultValue={undefined}>
        <AccordionItem value="details" className="border rounded-md px-3">
          <AccordionTrigger>Open 4.9.3 Enhancement Workspace</AccordionTrigger>
