...
export default function FranchiseConfigForm({ tenantIdOverride, franchiseIdOverride }: Props) {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        ...
        if (data) {
          const cfg = data as FranchiseConfig;
          setConfig(cfg);
          setForm({
            prefix: cfg.prefix || '',
            suffix: cfg.suffix || '',
        ...
        toast.error('Failed to load franchise config', { description: err.message });
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (error) throw error;
      setConfig(null);
      toast.success('Franchise config deleted');
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>Franchise Config</CardTitle>
      </CardHeader>
      <CardContent className="space-y-4">
        ...
        <div className="flex items-center gap-2">
          <Button disabled={!franchiseId || loading} onClick={handleSave}>{config ? 'Save Changes' : 'Create Config'}</Button>
