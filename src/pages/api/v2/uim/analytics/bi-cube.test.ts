...
vi.mock('../../../_utils/http', async () => {
  ...
  return {
    ...actual,
    applyCors: vi.fn(),
    buildApiContext: vi.fn(() => ({ correlationId: 'corr-uim-analytics-bi-cube' })),
    enforceHttps: vi.fn(),
...
describe('/api/v2/uim/analytics/bi-cube', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it('returns deployment artifact and published data dictionary', async () => {
    ...
    expect((res.jsonBody as any)?.output?.deployment_artifact?.artifact_id).toMatch(/^uim-bi-cube-/);
    expect((res.jsonBody as any)?.output?.data_dictionary?.publication_status).toBe('published');
  });
});
