...
export interface PersistedWorkOrder {
  id: string;
  tenant_id: string;
...
export interface WorkOrderTitleCatalogItem {
  id: string;
  title: string;
  wp_title: string;
  tenant_id: string;
...
async function resolveAircraftRegistration(
  ...
  const { data, error } = await supabase
    ...
    .maybeSingle();
  if (error) {
    logger.warn('amro-work-order-aircraft-registration-lookup-failed', { message: error.message, aircraftId });
    return 'UNKNOWN';
  }
...
type WorkOrderTitleRecord = { id: string; title: string; wp_title: string };

async function resolveWorkOrderTitleRecord(
  supabase: ReturnType<typeof getSupabaseAdminClient>,
  params: {
    tenantId: string;
    franchiseId: string | null;
    workOrderTitleId?: string;
    fallbackTitle: string;
  },
): Promise<WorkOrderTitleRecord> {
  const selectedTitleId = String(params.workOrderTitleId || '').trim();
  if (selectedTitleId) {
...
function toSafeWorkOrderSegment(value: string): string {
...
async function createTemplateTasksWithSupabase(params: {
  ...
  const taskTemplates = parseTaskTemplateArray(templateRow.tasks_json, params.workOrderTemplateId);
...
async function createTemplateTasksWithPgTransaction(params: {
  ...
  const duplicateGuardSql = `
    ...
    WHERE tenant_id = $1
      ...
      AND deleted_at IS NULL
  `;
  const duplicateGuardResult = await params.tx.query(duplicateGuardSql, [params.tenantId, params.workOrderId]);
...
function parseWorkOrderNumberSequence(workOrderNumber: string, year: number): number {
  const pattern = /^WP-(.+)-(\d{4})-(\d+)-([A-Z0-9-]+)$/;
  const match = String(workOrderNumber || '').match(pattern);
  if (!match) return 0;
...
async function getNextWorkOrderNumber(
  ...
  const maxSeq = (Array.isArray(data) ? data : []).reduce((max, row) => {
    const current = parseWorkOrderNumberSequence(String((row as Record<string, unknown>).work_order_number || ''), params.year);
    return current > max ? current : max;
  }, 0);
  const nextSeq = String(maxSeq + 1).padStart(4, '0');
  return `WP-${toSafeWorkOrderSegment(params.aircraftRegistration)}-${params.year}-${nextSeq}-${toSafeWorkOrderSegment(params.wpTitleCode)}`;
}

// ── Create work package ─────────────────────────────────────────────────────

export async function persistCreateWorkOrder(params: {
  tenantId: string;
  franchiseId: string | null;
  userId: string;
  aircraftId: string | null;
  ...
  workOrderTemplateId?: string;
  workOrderTitleId?: string;
}): Promise<PersistedWorkOrder> {
  const supabase = getSupabaseAdminClient();
  const titleRecord = await resolveWorkOrderTitleRecord(supabase, {
    tenantId: params.tenantId,
    franchiseId: params.franchiseId,
    workOrderTitleId: params.workOrderTitleId,
  ...
  const pool = params.workOrderTemplateId ? getDbPool() : null;
  if (pool) {
    ...
    try {
      ...
      const insertResult = await client.query(insertSql, [
        ...
        params.priority,
        params.source || null,
        params.workOrderTemplateId || null,
        titleRecord.id || null,
      ...
      const createdWorkOrder = insertResult.rows[0];
      if (!createdWorkOrder?.id) {
        throw new Error('Failed to persist work package in atomic transaction');
      }

      let generatedTasksCount = 0;
      if (params.workOrderTemplateId) {
        generatedTasksCount = await createTemplateTasksWithPgTransaction({
          tx: client,
          tenantId: params.tenantId,
          franchiseId: params.franchiseId,
          userId: params.userId,
          workOrderId: String(createdWorkOrder.id),
          workOrderNumber: String(createdWorkOrder.work_order_number || workOrderNumber),
          workOrderTemplateId: params.workOrderTemplateId,
        });
      }

      await client.query('COMMIT');
      logger.info('amro-work-order-created-atomic', {
        id: String(createdWorkOrder.id),
        workOrderNumber: String(createdWorkOrder.work_order_number || workOrderNumber),
        generatedTasksCount,
        tenantId: params.tenantId,
      });
      const mapped = mapRowToWorkOrder(createdWorkOrder);
      mapped.generated_tasks_count = generatedTasksCount;
      return mapped;
    } catch (error) {
      await client.query('ROLLBACK');
      logger.error('amro-work-order-atomic-create-failed', {
        tenantId: params.tenantId,
        workOrderTemplateId: params.workOrderTemplateId || null,
  ...
  const { data, error } = await supabase
    ...
    .insert({
      ...
      work_order_template_id: params.workOrderTemplateId || null,
  ...
  if (error) {
    logger.error('amro-work-order-persist-create-failed', {
  ...
  if (params.workOrderTemplateId) {
    try {
      generatedTasksCount = await createTemplateTasksWithSupabase({
        ...
        workOrderNumber: String(data.work_order_number || workOrderNumber),
        workOrderTemplateId: params.workOrderTemplateId,
      });
    } catch (taskCreationError) {
      logger.error('amro-work-order-template-task-create-failed', {
        tenantId: params.tenantId,
        workOrderId: String(data.id || ''),
        workOrderTemplateId: params.workOrderTemplateId,
      ...
      if (rollbackError) {
        logger.error('amro-work-order-template-task-create-rollback-failed', {
          tenantId: params.tenantId,
          workOrderId: String(data.id || ''),
  ...
  logger.info('amro-work-order-created', {
    id: data.id,
  ...
  const mapped = mapRowToWorkOrder(data);
  mapped.generated_tasks_count = generatedTasksCount;
  return mapped;
}
...
export async function persistUpdateWorkOrder(params: {
  id: string;
  tenantId: string;
  userId: string;
  ...
  if (error) {
    logger.error('amro-work-order-persist-update-failed', {
      message: error.message,
      code: error.code,
      id: params.id,
      tenantId: params.tenantId,
  ...
  return mapRowToWorkOrder(data);
}

// ── Transition work package status ──────────────────────────────────────────

export async function persistTransitionWorkOrder(params: {
  id: string;
  tenantId: string;
  userId: string;
  ...
  if (error) {
    logger.error('amro-work-order-transition-failed', {
      message: error.message,
      code: error.code,
      id: params.id,
  ...
  logger.info('amro-work-order-transitioned', {
    id: data.id,
    workOrderNumber: data.work_order_number,
...
export async function persistDeleteWorkOrder(params: {
  id: string;
  tenantId: string;
}): Promise<void> {
  const supabase = getSupabaseAdminClient();
  ...
  if (error) {
    logger.error('amro-work-order-delete-failed', {
      message: error.message,
      code: error.code,
      id: params.id,
      tenantId: params.tenantId,
  ...
  logger.info('amro-work-order-deleted', {
    id: params.id,
    tenantId: params.tenantId,
  });
}
...
export async function fetchWorkOrderList(params: {
  tenantId: string;
  franchiseId: string | null;
  page: number;
  pageSize: number;
  ...
  assignedTo?: string;
  search?: string;
}): Promise<{ rows: PersistedWorkOrder[]; total: number }> {
  const supabase = getSupabaseAdminClient();
  ...
  if (error) {
    logger.error('amro-work-order-list-failed', {
      message: error.message,
      code: error.code,
      tenantId: params.tenantId,
    });
  ...
  return {
    rows: (data || []).map(mapRowToWorkOrder),
    total: count || 0,
  };
}
...
export async function fetchWorkOrderDetail(params: {
  id: string;
  tenantId: string;
}): Promise<{
  ...
  if (tasksError) {
    logger.warn('amro-work-order-tasks-fetch-failed', {
  ...
  if (materialsError && /amro_work_order_materials|work_order_id/i.test(String(materialsError.message || ''))) {
    let legacyMatQuery = supabase.from('work_order_materials').select('*').eq('work_order_id', params.id);
  ...
  if (materialsError) {
    logger.warn('amro-work-order-materials-fetch-failed', {
  ...
  let evtQuery = supabase.from('maintenance_events').select('*').eq('work_order_id', params.id);
  ...
  if (eventsError) {
    logger.warn('amro-work-order-events-fetch-failed', {
  ...
  return {
    workOrder: mapRowToWorkOrder(wp),
...
function mapRowToWorkOrder(row: Record<string, unknown>): PersistedWorkOrder {
  return {
    id: String(row.id || ''),
    ...
    work_order_number: String(row.work_order_number || row.work_order_number || ''),
    ...
    status: (row.status as WorkOrderStatus) || 'planning',
...
export async function fetchWorkOrderTitleCatalog(params: {
  tenantId: string;
  franchiseId: string | null;
}): Promise<{ items: WorkOrderTitleCatalogItem[]; total: number }> {
  const supabase = getSupabaseAdminClient();
  let query = supabase
    .from('work_orders_title')
