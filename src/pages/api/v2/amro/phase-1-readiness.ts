...
import { buildAmroPhase1ReadinessEnvelope } from './phase-1-core-workflows';
...
function isV2Enabled(): boolean {
  return parseBoolean(process.env.AMRO_PHASE_1_READINESS_V2_ENABLED, true);
}
  ...
  try {
    ...
    if (!isV2Enabled()) {
      return res.status(404).json({
        error: 'AMRO phase-1-readiness v2 endpoint is disabled',
