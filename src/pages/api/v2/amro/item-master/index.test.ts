...
describe('/api/v2/amro/item-master', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    vi.mocked(handlePreflight).mockReturnValue(false);
    vi.mocked(buildApiContext).mockReturnValue({ correlationId: 'corr-item-master-index' } as any);
  ...
  it('returns paginated item master records for GET', async () => {
    const supabase: any = {
    ...
    expect((res.jsonBody as any)?.interface).toBe('amro-item-master-list');
  });

  it('creates an item master record for POST', async () => {
    const supabase: any = {
      from: vi.fn((table: string) => {
        if (table === 'amro_item_master') {
          return {
    ...
    expect((res.jsonBody as any)?.interface).toBe('amro-item-master-create');
  });
});
