...
describe('work package operation latency benchmark', () => {
    ...
    vi.mocked(getSupabaseAdminClient).mockReturnValue({
      schema: vi.fn(() => ({
        rpc: vi.fn(async (name: string) => {
          if (name === 'amro_ops_clone_template_work_order') {
            return {
              data: [{
          ...
          return {
            data: [{
              ...
              status: name === 'amro_ops_transition_work_order' ? 'in_progress' : 'planning',
              version: 2,
              created_at: '2026-03-24T00:00:00.000Z',
              created_by: 'user-1',
  ...
  it('keeps create-work-order median write latency below 100ms', async () => {
    const sample = await benchmarkOperation(async () => {
      await persistCreateWorkOrder({
        tenantId: 'tenant-1',
        franchiseId: 'fr-1',
        userId: 'user-1',
        aircraftId: 'ac-001',
  ...
  it('keeps transition-work-order median write latency below 100ms', async () => {
    const sample = await benchmarkOperation(async () => {
      await persistTransitionWorkOrder({
        tenantId: 'tenant-1',
        franchiseId: 'fr-1',
        userId: 'user-1',
        workOrderId: 'wp-001',
  ...
  it('keeps clone-template median write latency below 100ms', async () => {
    const sample = await benchmarkOperation(async () => {
      await persistCloneTemplateWorkOrder({
        tenantId: 'tenant-1',
        franchiseId: 'fr-1',
        userId: 'user-1',
