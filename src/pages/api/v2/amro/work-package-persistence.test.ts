...
describe('work-package-persistence', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it('persists create work package through amro_ops rpc', async () => {
    ...
    const record = await persistCreateWorkPackage({
      tenantId: 'tenant-1',
      franchiseId: 'fr-1',
      userId: 'user-1',
  ...
  it('returns optimistic lock conflict when transition rpc reports version mismatch', async () => {
    ...
    await expect(persistTransitionWorkPackage({
      tenantId: 'tenant-1',
      franchiseId: 'fr-1',
      userId: 'user-1',
  ...
  it('supports concurrency control by allowing one transition and rejecting stale version', async () => {
    ...
    const [first, second] = await Promise.allSettled([
      persistTransitionWorkPackage(input),
      persistTransitionWorkPackage(input),
    ]);
  ...
  it('propagates clone-template persistence failures for rollback-safe handling', async () => {
    ...
    await expect(persistCloneTemplateWorkPackage({
      tenantId: 'tenant-1',
      franchiseId: 'fr-1',
      userId: 'user-1',
