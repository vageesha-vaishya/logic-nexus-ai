...
import { getEngineConfigurationGraph } from '../../engine-read-model-store';
...
export default async function handler(req: ApiRequest, res: ApiResponse) {
  ...
  try {
    ...
    const output = getEngineConfigurationGraph({ tenantId, franchiseId }, engineId);
    res.status(200).json({ version: 'v2', interface: 'get-engine-configuration-graph', correlationId: ctx.correlationId, output });
  } catch (error) {
