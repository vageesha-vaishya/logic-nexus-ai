...
type TransitionWorkPackageInput = {
  tenantId: string;
  franchiseId: string;
  userId: string;
...
export type PersistedWorkPackage = {
...
function parseRpcResponse(data: unknown): PersistedWorkPackage {
  ...
  return {
    work_package_id: String(record.work_package_id || ''),
...
export async function persistCreateWorkPackage(input: CreateWorkPackageInput): Promise<PersistedWorkPackage> {
  const supabase = getSupabaseAdminClient().schema('amro_ops');
  const { data, error } = await supabase.rpc('amro_ops_create_work_package', {
...
export async function persistTransitionWorkPackage(input: TransitionWorkPackageInput): Promise<PersistedWorkPackage> {
  const supabase = getSupabaseAdminClient().schema('amro_ops');
  const { data, error } = await supabase.rpc('amro_ops_transition_work_package', {
    p_tenant_id: input.tenantId,
    p_franchise_id: input.franchiseId,
    p_user_id: input.userId,
...
export async function persistCloneTemplateWorkPackage(input: CloneWorkPackageInput): Promise<PersistedWorkPackage> {
  const supabase = getSupabaseAdminClient().schema('amro_ops');
  const { data, error } = await supabase.rpc('amro_ops_clone_template_work_package', {
    p_tenant_id: input.tenantId,
    p_franchise_id: input.franchiseId,
    p_user_id: input.userId,
