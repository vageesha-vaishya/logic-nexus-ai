...
import handler from './work-orders';
...
describe('/api/v2/amro/work-orders', () => {
  ...
  beforeEach(() => {
    ...
    vi.mocked(persistTransitionWorkOrder).mockResolvedValue({
    ...
    vi.mocked(persistCloneTemplateWorkOrder).mockResolvedValue({
      work_order_id: 'tenant-1-fr-1-wp-clone-100',
      status: 'planning',
      version: 1,
      created_at: '2026-03-21T00:00:00.000Z',
      created_by: 'user-1',
  ...
  it('returns dual-run response when feature and dual-run flags are enabled', async () => {
    ...
    expect((res.jsonBody as any)?.data?.workOrders?.[0]?.domainId).toBe('amro');
    expect((res.jsonBody as any)?.data?.workOrders?.[0]?.version).toBe('v2');
  ...
  it('returns legacy fallback payload when fallback flag is enabled', async () => {
    ...
    expect((res.jsonBody as any)?.data?.workOrders?.[0]?.id).toContain('legacy-');
    expect((res.jsonBody as any)?.auditLedger?.recordId).toBeTruthy();
  });
  ...
  it('creates work package with 15.2.2 create interface contract', async () => {
    ...
    expect(res.statusCode).toBe(200);
    expect((res.jsonBody as any)?.interface).toBe('create-work-order');
  ...
  it('returns persistence error and skips runtime fallback when template task generation fails', async () => {
    process.env.AMRO_WORK_PACKAGES_V2_ENABLED = 'true';
    vi.mocked(persistCreateWorkOrder).mockRejectedValueOnce(new Error('task_template_id not found (bad-id)'));
    const req: ApiRequest = {
      method: 'POST',
      query: { interface: 'create-work-order' },
      body: {
  ...
  it('saves work package view with filters', async () => {
    ...
    const req: ApiRequest = {
      method: 'POST',
      query: { interface: 'save-work-order-view' },
      body: {
    ...
    expect(res.statusCode).toBe(200);
    expect((res.jsonBody as any)?.interface).toBe('save-work-order-view');
  ...
  it('delegates transition role-policy rejection from persistence layer', async () => {
    ...
    vi.mocked(persistTransitionWorkOrder).mockRejectedValueOnce(new Error('transition is not allowed for role'));
    const req: ApiRequest = {
      method: 'POST',
      query: { interface: 'transition-work-order' },
      body: {
        work_order_id: 'wp-001',
  ...
  it('rejects transition when target status is invalid', async () => {
    ...
    const req: ApiRequest = {
      ...
      query: { interface: 'transition-work-order' },
      body: {
        work_order_id: 'wp-invalid-status-001',
    ...
    expect(persistTransitionWorkOrder).not.toHaveBeenCalled();
  });
  ...
  it('publishes lifecycle closure event when transition reaches completed', async () => {
    ...
    expect((res.jsonBody as any)?.output?.published_events?.[0]?.event_type).toBe('amro.work_order.lifecycle.closed.v1');
    ...
    expect(persistTransitionWorkOrder).toHaveBeenCalledWith(expect.objectContaining({
      actorRole: 'inspector',
  ...
  it('returns conflict when optimistic lock check fails on transition', async () => {
    ...
    vi.mocked(persistTransitionWorkOrder).mockRejectedValueOnce(new Error('optimistic_lock_conflict'));
    const req: ApiRequest = {
      method: 'POST',
      query: { interface: 'transition-work-order' },
      body: {
        work_order_id: 'wp-closure-rollback-001',
        current_status: 'in_progress',
        target_status: 'completed',
        reason_code: 'all-gates-passed',
  ...
  it('clones template using registry-backed template access checks', async () => {
    ...
    expect((res.jsonBody as any)?.output?.new_work_order_id).toBe('tenant-1-fr-1-wp-clone-100');
  ...
  it('rejects clone-template when registry denies active template access', async () => {
    ...
    expect(persistCloneTemplateWorkOrder).not.toHaveBeenCalled();
  });
  ...
  it('rejects reserve-parts when work package scope does not match tenant scope', async () => {
    ...
    const req: ApiRequest = {
      ...
      query: { interface: 'reserve-parts' },
      body: {
        work_order_id: 'tenant-999:wp-001',
  ...
  it('syncs supplier ETA only from trusted adapters with valid datetime', async () => {
    ...
    const req: ApiRequest = {
      ...
      body: {
        ...
        impacted_work_orders: ['wp-001', 'wp-002'],
      },
      headers: {},
    };
    const res = createResponse();
    ...
    expect((res.jsonBody as any)?.output?.impacted_work_orders).toEqual(['wp-001', 'wp-002']);
  });
  ...
  it('syncs supplier ASN with ERP procurement when M7 is completed', async () => {
    ...
    expect((res.jsonBody as any)?.output?.impacted_work_orders).toEqual(['wp-001']);
  });
  ...
  it('returns readiness, compliance gates, and optimization board query contracts', async () => {
    ...
    const readinessReq: ApiRequest = {
      method: 'GET',
      query: { interface: 'readiness', work_order_id: 'wp-001' },
      headers: { 'x-api-version': 'v2' },
    };
    ...
    const gatesReq: ApiRequest = {
      method: 'GET',
      query: { interface: 'compliance-gates', work_order_id: 'wp-001' },
      headers: { 'x-api-version': 'v2' },
    };
