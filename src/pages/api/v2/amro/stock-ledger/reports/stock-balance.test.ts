...
import handler from './stock-balance';
import {
  authenticateRequest,
  buildApiContext,
...
describe('/api/v2/amro/stock-ledger/reports/stock-balance', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    vi.mocked(handlePreflight).mockReturnValue(false);
    vi.mocked(buildApiContext).mockReturnValue({ correlationId: 'corr-stock-balance-report' } as any);
  ...
  it('returns stock balance report on GET', async () => {
    const queryBuilder: any = {
      eq: vi.fn(() => queryBuilder),
      order: vi.fn(() => queryBuilder),
      then: (resolve: (value: unknown) => void) => resolve({
        data: [{ part_inventory_id: 'p1', ledger_quantity_on_hand: 100 }],
        error: null,
      }),
    };
    const supabase: any = {
    ...
    expect((res.jsonBody as any)?.interface).toBe('amro-stock-ledger-report-stock-balance');
  });
});
