...
import handler from './valuation-summary';
import {
  authenticateRequest,
  buildApiContext,
...
describe('/api/v2/amro/stock-ledger/reports/valuation-summary', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    vi.mocked(handlePreflight).mockReturnValue(false);
    vi.mocked(buildApiContext).mockReturnValue({ correlationId: 'corr-valuation-summary-report' } as any);
  ...
  it('returns valuation summary on GET', async () => {
    const queryBuilder: any = {
      eq: vi.fn(() => queryBuilder),
      then: (resolve: (value: unknown) => void) => resolve({
        data: [{ valuation_method: 'weighted_average', currency: 'USD', inventory_value: 1000 }],
        error: null,
      }),
    };
    const supabase: any = {
    ...
    expect((res.jsonBody as any)?.interface).toBe('amro-stock-ledger-report-valuation-summary');
    expect((res.jsonBody as any)?.output.summary).toHaveProperty('weighted_average');
  });
});
