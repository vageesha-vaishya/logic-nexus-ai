...
export default async function handler(req: ApiRequest, res: ApiResponse): Promise<void> {
  ...
  try {
    ...
    const summary = (data || []).reduce<Record<string, { currency: string; total_value: number; rows: number }>>((acc, row) => {
      ...
      const value = Number((row as any).inventory_value || 0);
      if (!acc[method]) acc[method] = { currency, total_value: 0, rows: 0 };
      acc[method].total_value += value;
      acc[method].rows += 1;
      return acc;
    }, {});
    res.status(200).json({
      ...
      interface: 'amro-stock-ledger-report-valuation-summary',
      output: { summary },
    });
  } catch (error) {
