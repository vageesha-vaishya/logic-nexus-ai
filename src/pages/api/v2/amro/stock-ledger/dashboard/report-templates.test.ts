...
import handler from './report-templates';
import {
  authenticateRequest,
  buildApiContext,
...
describe('/api/v2/amro/stock-ledger/dashboard/report-templates', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    vi.mocked(handlePreflight).mockReturnValue(false);
    vi.mocked(buildApiContext).mockReturnValue({ correlationId: 'corr-report-templates' } as any);
  ...
  it('creates a report template', async () => {
    const supabase: any = {
      from: vi.fn(() => ({
        select: vi.fn(() => ({
          eq: vi.fn(() => ({
            maybeSingle: vi.fn().mockResolvedValue({ data: { tenant_id: 'tenant-1', emergency_contact_info: {} }, error: null }),
          })),
        })),
        update: vi.fn(() => ({
    ...
    const req: ApiRequest = {
      ...
      query: {},
      headers: {},
      body: { name: 'Weekly Snapshot', report_type: 'stock-balance' },
    };
    const res = createResponse();
    await handler(req, res);
    expect(res.statusCode).toBe(201);
    expect((res.jsonBody as any)?.interface).toBe('amro-stock-ledger-report-template-create');
  });
});
