...
import { buildAmroModuleCatalogEnvelope } from './module-catalog-model';
...
function isV2Enabled(): boolean {
  return parseBoolean(process.env.AMRO_MODULE_CATALOG_V2_ENABLED, true);
}
  ...
  try {
    ...
    if (!isV2Enabled()) {
      return res.status(404).json({
        error: 'AMRO module-catalog v2 endpoint is disabled',
    ...
    const moduleCatalog = buildAmroModuleCatalogEnvelope();
