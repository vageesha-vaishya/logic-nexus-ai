...
import { buildAmroScreenInventoryEnvelope } from './screen-inventory-model';
...
function isV2Enabled(): boolean {
  return parseBoolean(process.env.AMRO_SCREEN_INVENTORY_V2_ENABLED, true);
}
  ...
  try {
    ...
    if (!isV2Enabled()) {
      return res.status(404).json({
        error: 'AMRO screen-inventory v2 endpoint is disabled',
    ...
    const screenInventory = buildAmroScreenInventoryEnvelope();
