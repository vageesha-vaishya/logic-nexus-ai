...
export default async function handler(req: ApiRequest, res: ApiResponse): Promise<void> {
  ...
  try {
    ...
    if (readError) {
      throw new Error(`Failed to query directives record: ${readError.message}`);
    }
    if (!current) {
    ...
    if (req.method === 'GET') {
      res.status(200).json({
        ...
        interface: 'amro-directives-detail',
        output: {
          record: mapDirectiveRowToRecord(existing),
        },
      });
      return;
    }
    ...
    if (req.method === 'DELETE') {
      ...
      if (deleteError) {
        throw new Error(`Failed to delete directives record: ${deleteError.message}`);
      }
    ...
    const issues = validateDirectiveInput(payload, 'patch');
    if (issues.length > 0) {
      res.status(400).json({ error: 'Validation failed', issues, version: 'v2', correlationId: ctx.correlationId });
      return;
    }

    const patch = mapPayloadToDirectiveInput(payload);
    ...
    if (updateError) {
      throw new Error(`Failed to update directives record: ${updateError.message}`);
    }
    ...
    res.status(200).json({
      ...
      output: {
        record: mapDirectiveRowToRecord((updated || {}) as Record<string, unknown>),
      },
    });
  } catch (error) {
