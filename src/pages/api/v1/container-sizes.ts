...
export default async function handler(req: ApiRequest, res: ApiResponse) {
  ...
  try {
    ...
    const data = await service.getContainerSizes(ctx.tenantId, typeId || undefined);

    if (!data.length) {
      return res.status(404).json({ error: 'No container sizes found', correlationId: ctx.correlationId });
    }

    logApiEvent('info', '[ContainerAPI] container-sizes fetched', {
    ...
    logApiEvent('error', '[ContainerAPI] container-sizes failed', {
      correlationId: ctx.correlationId,
      tenantId: ctx.tenantId,
