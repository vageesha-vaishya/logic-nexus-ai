...
create table if not exists amro_ops.work_order (
...
create or replace function amro_ops.amro_ops_transition_work_order(
  p_tenant_id text,
  p_franchise_id text,
  p_user_id text,
...
create or replace function amro_ops.amro_ops_clone_template_work_order(
  p_tenant_id text,
  p_franchise_id text,
  p_user_id text,
  p_template_id text,
...
as $$
declare
  v_work_order_id text := p_tenant_id || '-' || p_franchise_id || '-wp-clone-' || extract(epoch from clock_timestamp())::bigint::text;
begin
...
grant execute on function amro_ops.amro_ops_create_work_order(
...
grant execute on function amro_ops.amro_ops_transition_work_order(
  text, text, text, text, text, text, text, text, integer, text, text, text, jsonb, jsonb
) to authenticated;
grant execute on function amro_ops.amro_ops_clone_template_work_order(
