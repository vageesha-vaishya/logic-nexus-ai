...
begin
  if exists (
    ...
    select 1
    from pg_constraint
    where conname = 'fk_amro_ops_work_order_template_id'
      and conrelid = 'amro_ops.work_order'::regclass
  ) then
    alter table amro_ops.work_order
...
create table if not exists amro_ops.work_order_task (
  task_id text primary key,
  work_order_id text not null references amro_ops.work_order(work_order_id) on delete cascade,
...
create index if not exists idx_amro_ops_work_order_task_wp_id
  on amro_ops.work_order_task(work_order_id);

create index if not exists idx_amro_ops_work_order_task_template_id
  on amro_ops.work_order_task(task_template_id);
...
begin
  if exists (
    ...
    select 1
    from pg_constraint
    where conname = 'fk_amro_ops_work_order_task_template_id'
      and conrelid = 'amro_ops.work_order_task'::regclass
  ) then
    alter table amro_ops.work_order_task
      add constraint fk_amro_ops_work_order_task_template_id
...
create or replace function amro_ops.amro_ops_create_work_order(
  p_tenant_id text,
  p_franchise_id text,
  p_user_id text,
  p_work_order_template_id text,
  p_aircraft_id text,
...
as $$
declare
  v_work_order_id text := p_tenant_id || '-' || p_franchise_id || '-wp-' || extract(epoch from clock_timestamp())::bigint::text;
  v_template_id uuid;
...
begin
  ...
  if nullif(trim(coalesce(p_work_order_template_id, '')), '') is not null then
    begin
      v_template_id := trim(p_work_order_template_id)::uuid;
    exception
      when others then
        raise exception 'work_order_template_id is invalid';
    end;

    if to_regclass('public.work_order_templates') is null then
      raise exception 'work_order_templates table is not available';
    end if;

    select coalesce(wpt.tasks_json, '[]'::jsonb)
    into v_tasks_json
    from public.work_order_templates wpt
  ...
  insert into amro_ops.work_order (
    work_order_id,
    tenant_id,
    franchise_id,
  ...
  if jsonb_array_length(v_tasks_json) > 0 then
    ...
    loop
      ...
      loop
        v_seq := v_seq + 1;
        insert into amro_ops.work_order_task (
          task_id,
          work_order_id,
          tenant_id,
          franchise_id,
          ...
          v_work_order_id || '-tsk-' || lpad(v_seq::text, 4, '0'),
          v_work_order_id,
          p_tenant_id,
          p_franchise_id,
          v_task_template_id,
  ...
  if to_regclass('amro_audit.amro_workflow_tx_log') is not null then
    ...
    values (
      ...
      'wp-create-' || v_work_order_id,
      'work-order-create-task-generation',
      jsonb_build_object(
        'work_order_id', v_work_order_id,
        'work_order_template_id', v_template_id,
  ...
  from amro_ops.work_order wp
  where wp.work_order_id = v_work_order_id;
exception
  when others then
    if to_regclass('amro_audit.amro_workflow_tx_log') is not null then
      insert into amro_audit.amro_workflow_tx_log (
      ...
      values (
        ...
        'wp-create-' || coalesce(v_work_order_id, 'unknown'),
        'work-order-create-task-generation',
        jsonb_build_object(
          'work_order_template_id', p_work_order_template_id,
          'tenant_id', p_tenant_id,
          'franchise_id', p_franchise_id
        )::text,
        jsonb_build_object(
