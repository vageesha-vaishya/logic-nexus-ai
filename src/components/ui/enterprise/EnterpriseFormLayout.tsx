...
export function EnterpriseFormLayout({
  ...
  return (
    <div className={cn("flex flex-col h-full bg-muted", className)}>
      ...
      <div className="bg-card border-b px-4 py-2 flex items-center justify-between sticky top-0 z-20 shadow-sm h-14 shrink-0">
              ...
              <nav className="flex items-center gap-1 text-sm text-muted-foreground">
                {breadcrumbs.map((crumb, i) => (
                  <React.Fragment key={i}>
                    ...
                    {crumb.to ? (
                      <Link to={crumb.to} className="hover:text-primary transition-colors text-foreground font-medium">
                        {crumb.label}
                      </Link>
                    ) : (
