...
describe('AMRO Aircraft Dashboard API', () => {
  it('should resolve engine operations dashboard without Not Found when tenant scope headers are provided in test mode', async () => {
    ...
    if (response.body.output.aircraft_status.length > 0) {
      ...
      expect(Object.prototype.hasOwnProperty.call(response.body.output.aircraft_status[0], 'aircraft_model')).toBe(true);
      expect(Object.prototype.hasOwnProperty.call(response.body.output.aircraft_status[0], 'serial_number')).toBe(true);
    }
