...
async function createTestApp(fromMock: jest.Mock) {
  ...
  const { default: router } = await import('../src/routes/work-order-template.routes');
  const app = express();
...
describe('work-order-template.routes task-template relationship endpoint', () => {
  beforeEach(() => {
    jest.clearAllMocks();
  });
  ...
  it('creates relationship rows for a template and returns 201', async () => {
    ...
    const response = await request(app)
      .post('/api/v2/work-order-templates/aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa/task-templates')
...
describe('work-order-template.routes create endpoint', () => {
  beforeEach(() => {
    jest.clearAllMocks();
  });
  ...
  it('creates template + relationships and returns created template id with relationship_count', async () => {
    ...
    const app = await createTestApp(fromMock);
    const response = await request(app)
      .post('/api/v2/work-order-templates')
      .send({
        template_code: 'WP-LINE-001',
        template_name: 'Line Check Package',
        maintenance_type: 'line',
...
describe('work-order-template.routes underscore path and PATCH alias compatibility', () => {
  beforeEach(() => {
    jest.clearAllMocks();
  });

  it('accepts PATCH on /api/v2/amro/work_order_templates/:id and routes to template update handler', async () => {
    const app = await createTestApp(jest.fn());
    const response = await request(app)
      .patch('/api/v2/amro/work_order_templates/aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa')
      .send({})
      .expect(422);
