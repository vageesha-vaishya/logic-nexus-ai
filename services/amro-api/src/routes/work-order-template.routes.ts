...
router.use((req, _res, next) => {
  if (req.url.startsWith('/amro/work-order-templates')) {
    req.url = req.url.replace('/amro/work-order-templates', '/work-order-templates');
  }
  if (req.url.startsWith('/amro/work_order_templates')) {
    req.url = req.url.replace('/amro/work_order_templates', '/work-order-templates');
  }
  if (req.url.startsWith('/work_order_templates')) {
    req.url = req.url.replace('/work_order_templates', '/work-order-templates');
  }
...
type CreateWorkOrderTemplateTaskTemplateRequest = {
...
function isMissingWorkOrderTemplateFranchiseColumnError(errorLike: unknown): boolean {
  ...
  return message.includes('work_order_templates.franchise_id')
    || message.includes('work_order_templates.franchise_id')
    || (message.includes('franchise_id') && (message.includes('work_order_templates') || message.includes('work_order_templates')));
}
...
function normalizeTaskTemplateRequestIds(input: CreateWorkOrderTemplateTaskTemplateRequest): string[] {
...
function normalizeTemplateRequestTaskTemplateIds(input: WorkOrderTemplateRequest): string[] {
  const direct = normalizeTaskTemplateIds(input.selected_task_template_ids);
  if (direct.length > 0) return direct;
...
async function createWorkOrderTemplateFromRequest(req: AuthRequest, res: { status: (code: number) => { json: (body: unknown) => void } }): Promise<void> {
  ...
  const request = (req.body || {}) as WorkOrderTemplateRequest;
  ...
  const { data: insertedTemplate, error: insertError } = await supabase
    ...
    .single();
  if (insertError) {
    logger.error('[WorkOrderTemplateRoutes] create failed', {
      tenantId,
...
async function createTemplateTaskRelationshipsFromRequest(
  ...
  const userId = req.userId;
  const franchiseId = getFranchiseId(req);
  const { workOrderTemplateId } = req.params as { workOrderTemplateId: string };
  if (!tenantId || !userId) {
    res.status(401).json(toErrorResponse('Missing tenant or user context', 'MISSING_CONTEXT', 401));
    return;
  }

  const request = (req.body || {}) as CreateWorkOrderTemplateTaskTemplateRequest;
  const resolvedTemplateId = String(request.work_order_template_id || workOrderTemplateId || '').trim();
  ...
  logger.info('[AMRO Work Package Template] add task-template relationships request', {
    ...
    franchiseId,
    userId,
    workOrderTemplateId: resolvedTemplateId,
  ...
  if (templateError && isMissingWorkOrderTemplateFranchiseColumnError(templateError)) {
...
async function loadWorkOrderTemplateModelOptions(
  tenantId: string,
  franchiseId: string | null,
): Promise<Array<Record<string, unknown>>> {
...
async function updateWorkOrderTemplateWithoutAtomicRpc(params: {
  tenantId: string;
  franchiseId: string | null;
  userId: string;
  templateId: string;
  ...
  if (updateError && isMissingWorkOrderTemplateFranchiseColumnError(updateError) && franchiseId) {
...
router.post(
  ...
  asyncHandler(async (req: AuthRequest, res, next): Promise<void> => {
    ...
    await createWorkOrderTemplateFromRequest(req, res);
  }),
);

router.post(
  '/amro/work-order-templates',
...
/**
 * @openapi
 * /api/v2/work-order-templates/{workOrderTemplateId}/task-templates:
 *   post:
 ...
 */
router.post(
  '/work-order-templates/:workOrderTemplateId/task-templates',
  asyncHandler(async (req: AuthRequest, res): Promise<void> => {
...
router.get(
  '/work-order-templates/model-options',
  asyncHandler(async (req: AuthRequest, res): Promise<void> => {
    const tenantId = req.tenantId;
    const franchiseId = getFranchiseId(req);
    if (!tenantId) {
    ...
    const records = await loadWorkOrderTemplateModelOptions(tenantId, franchiseId);
    res.json({ data: records, count: records.length });
    return;
  }),
);
...
router.get(
  '/work-order-templates/task-template-options',
  asyncHandler(async (req: AuthRequest, res): Promise<void> => {
    const tenantId = req.tenantId;
    const franchiseId = getFranchiseId(req);
...
router.put(
  ...
  asyncHandler(async (req: AuthRequest, res): Promise<void> => {
    ...
    const runFallbackUpdate = async () => {
      const fallbackResult = await updateWorkOrderTemplateWithoutAtomicRpc({
        tenantId,
        franchiseId,
        userId,
        templateId: id,
...
router.delete(
  ...
  asyncHandler(async (req: AuthRequest, res): Promise<void> => {
    ...
    if (error && isMissingWorkOrderTemplateFranchiseColumnError(error) && franchiseId) {
