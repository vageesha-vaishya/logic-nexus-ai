"""Portfolio AI Advisor + Attribution endpoints.
...
Retired 2026-05-26: GET /v1/portfolio/pnl/{portfolio_id} moved to the
`portfolio-pnl` Supabase Edge Function so the Sthira APK can load the
P&L cards over LTE without LAN routing. The transactions-replay algorithm
lives at supabase/functions/portfolio-pnl/replay.ts now; _BUY_TYPES /
_SELL_TYPES stay here because the /attribution monthly cash-flow code
still groups by them.
"""
