// Phase 7 UIM Step 4b.13 — analytics service carve.
//
// Verbatim copy of src/services/uim/uimAnalyticsService.ts (252 LOC).
// Carved into uim-api ahead of the 6 analytics routes (kpis, etl,
// reconciliation, bi-cube, qa-signoff, sla-evidence) so each route
// can land as its own slice without dragging the service module in
// with it.
//
// Behavior preserved byte-for-byte — the KPI definitions, semantic
// model, and computeUimAnalyticsKpis math (4-decimal rounding,
// turnover floor=1) are part of the contract surface.
...
export async function computeUimAnalyticsKpis(
  ...
  if (projectionError) {
    throw new Error(
      `Failed to load projection snapshots for analytics: ${projectionError.message || 'unknown error'}`,
    );
  }
  ...
  if (inventoryError) {
    throw new Error(
      `Failed to load inventory items for analytics: ${inventoryError.message || 'unknown error'}`,
    );
  }
  ...
  const inTransitItems = items.filter(
    (row) => String(row.status || '').toLowerCase() === 'in_transit',
  ).length;
