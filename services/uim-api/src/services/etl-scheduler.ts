// Phase 7 UIM Step 4b.14 — ETL scheduler service carve.
//
// Verbatim copy of src/modules/uim/analytics/etlScheduler.ts (294 LOC).
// In-memory queue with pluggable persistence adapter + executor.
// Default behavior:
//   - Persistence: in-process Map (does NOT survive restarts).
//   - Executor: no-op (returns 0/0/0).
//
// This is the legacy behavior preserved 1:1. Wiring the persistence
// adapter to a uim.etl_runs table is a follow-up architectural slice
// (same pattern as the webhooks DLQ rewrite).
//
// Exponential backoff on retry: 1s, 2s, 4s, … capped only by
// max_attempts. Default max_attempts=4.
...
export function getUimEtlTelemetrySummary(filter?: UimEtlQueueFilter): UimEtlTelemetrySummary {
  ...
  return {
    ...
    average_duration_ms: completedRuns.length > 0
      ? Number((totalDuration / completedRuns.length).toFixed(2))
      : 0,
    success_rate: Number(
      (completedRuns.length / Math.max(1, completedRuns.length + failedRuns.length)).toFixed(4),
    ),
