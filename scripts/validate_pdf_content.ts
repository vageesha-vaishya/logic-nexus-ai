...
// @ts-ignore -- PDFParse type definition is missing or incompatible
...
async function validate() {
    ...
    try {
        ...
        // @ts-ignore -- PDFParse might be a function
        const data = await PDFParse(dataBuffer);
        text = data.text;
    } catch (e: any) {
        console.log("Direct call failed, trying new:", e.message);
        try {
            // @ts-ignore -- PDFParse might be a class
            const instance: any = new PDFParse(dataBuffer);
