...
async function main() {
    // Check columns of quotation_version_options
    // We can't query information_schema easily via JS client usually, unless we use RPC or just try to select * and see keys.
    const { data, error } = await supabase.from('quotation_version_options').select('*').limit(1);
    if (error) {
        console.error(error);
    } else if (data && data.length > 0) {
        console.log('Columns:', Object.keys(data[0]));
    } else {
        console.log('Table empty or no access, but no error.');
    }
}
